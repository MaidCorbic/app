(() => {
  ...
  const state = { active:false, step:0, scene:null, completed:false, startX:0, actionBaseline:0, watcher:0, marker:null, timers:new Set() };
  ...
  style.textContent = `
    ...
    #relayTutorialOnboardingV3 .training-card{position:absolute;left:50%;bottom:max(28px,env(safe-area-inset-bottom) + 24px);transform:translateX(-50%);width:min(540px,calc(100vw - 32px));padding:18px 22px;border:1px solid rgba(113,241,255,.28);border-radius:18px;background:linear-gradient(145deg,rgba(5,20,35,.94),rgba(2,8,16,.97));box-shadow:0 24px 70px rgba(0,0,0,.55),0 0 44px rgba(83,230,255,.09);text-align:center;backdrop-filter:blur(12px)}
    .training-card small{display:block;color:#77ecff;font-size:8px;font-weight:900;letter-spacing:.24em}.training-card h2{margin:8px 0 8px;color:#f4fbff;font-size:clamp(23px,4vw,38px);line-height:1;letter-spacing:.04em}.training-card p{max-width:480px;margin:0 auto;color:#c6d6e3;font-size:12px;line-height:1.5}.training-card kbd{display:inline-flex;margin-top:12px;padding:7px 10px;border:1px solid rgba(137,239,255,.24);border-radius:8px;background:#061321;color:#9af5ff;font:800 9px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.15em}.training-progress{display:flex;justify-content:center;gap:5px;margin-top:13px}.training-progress i{width:27px;height:3px;background:rgba(133,180,201,.18)}.training-progress i.done{background:#72efff;box-shadow:0 0 10px rgba(114,239,255,.7)}.training-progress i.active{background:#a87cff}
    ...
    body.relay-training-active #intro,body.relay-training-active #relayGameplayIntroFinalV2,body.relay-training-active .hud,body.relay-training-active .world-marker,body.relay-training-active .input-guide,body.relay-training-active #relay-gameplay-new-layer,body.relay-training-active #pauseMenu,body.relay-training-active #finish,body.relay-training-active #gameOver,body.relay-training-active #toast{display:none!important;visibility:hidden!important;pointer-events:none!important}
    body.relay-training-active #relayTutorialOnboardingV3{pointer-events:none}
    body.relay-cinematic-active .hud,body.relay-cinematic-active .world-marker,body.relay-cinematic-active .input-guide,body.relay-cinematic-active .mobile-controls,body.relay-cinematic-active #pauseMenu,body.relay-cinematic-active #finish,body.relay-cinematic-active #gameOver,body.relay-cinematic-active #toast{display:none!important;visibility:hidden!important;pointer-events:none!important}
    @media(max-width:700px){#relayTutorialOnboardingV3 .training-map{left:50%;transform:translateX(-50%);top:max(10px,env(safe-area-inset-top));width:min(92vw,390px)}#relayTutorialOnboardingV3 .training-card{width:min(92vw,540px);padding:16px 14px}.training-card p{font-size:12px}.training-cinema-card{padding:28px 18px}.training-cinema-card h2{font-size:clamp(42px,12vw,64px)}.training-cinema-card p{font-size:12px}.training-cinema-card span{font-size:8px;line-height:1.5}}
  `;
  ...
  const persist = () => state.active && sessionStorage.setItem(STEP_KEY,String(state.step));
  ...
  const nearestTargetBarrier = (scene, direction=1) => {
    ...
    const candidates=(scene?.barriers?.getChildren?.()||[]).filter(b=>b?.active).map(b=>({b,dx:(b.x-px)*direction,dy:Math.abs(b.y-py)})).filter(x=>x.dx>=0&&x.dx<=520&&x.dy<=190).sort((a,b)=>a.dx-b.dx);
  ...
  const show = () => {
    ...
    root.querySelectorAll('.training-track span').forEach((n,i)=>n.className=i<state.step?'done':i===state.step?'active':''); q('.training-progress').innerHTML=steps.map((_,i)=>`<i class="${i<state.step?'done':i===state.step?'active':''}"></i>`).join('');
    if(state.step===2) armMarker(state.scene); else clearMarker(); persist();
  };
  const next = () => { if(!state.active) return; state.step++; if(state.step>=steps.length) finishTutorial(); else show(); };
  ...
  const finishTutorial = () => {
    ...
    state.active=false; cancelAnimationFrame(state.watcher); clearMarker(); sessionStorage.removeItem(STEP_KEY); setTrainingMode(false); setCinematicMode(true);
    q('.training-card').hidden=true; q('.training-map').hidden=true; q('.training-cinema').hidden=false; root.hidden=false;
    let done=false; const unlock=()=>{if(done)return;done=true;window.dispatchEvent(new Event('relay:cinematic-unlock'));q('.training-cinema').hidden=true;root.hidden=true;setCinematicMode(false);state.completed=true;sessionStorage.setItem(COMPLETE_KEY,'1');window.dispatchEvent(new CustomEvent('relay:tutorial-complete',{detail:{scene:state.scene}}));};
    window.dispatchEvent(new Event('relay:cinematic-lock')); state.timers.add(setTimeout(unlock,2600)); state.timers.add(setTimeout(unlock,4200));
  };
  const begin = scene => {
    ...
    state.active=true; setTrainingMode(true); show(); state.watcher=requestAnimationFrame(tick);
  };

  const ready = event => { const scene=event.detail?.scene||window.__relayRunnerScene; if(scene?.mission?.id!=='first-delivery') return; window.setTimeout(()=>begin(scene),120); };
  ...
  window.addEventListener('beforeunload',()=>{if(state.active)persist();clearMarker();});
})();