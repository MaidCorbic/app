...
(() => {
  ...
  if (speech && !speech.__relayAiVoiceGateV1) {
    ...
    speech.speak = utterance => {
      ...
      // Do not allow speech synthesis to start before the user has
      ...
      try {
        ...
        console.warn(
          '[Relay Runner] AI voice speak blocked:',
          error
        );
      }
    };
  ...
  prototype.init = function relayTutorialAwareInit(config = {}) {
    return originalInit.call(this, {
      ...config,
      firstTimeTutorial:
        tutorialEnabled() && Boolean(config.firstTimeTutorial)
    });
  };
  ...
  prototype.useGadget = function relayTutorialAwareGadget(...args) {
    ...
    events.emit = function gatedTutorialEmit(
      eventName,
      ...eventArgs
    ) {
      if (eventName === 'tutorial') {
        return this;
      }
    ...
    try {
      return originalUseGadget?.apply(
        this,
        args
      );
    } finally {
