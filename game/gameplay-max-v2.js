...
(() => {
  ...
  const createCache = (scene, x, y, index) => {
    ...
    if (!scene.motionReduced) {
      scene.tweens.add({ targets: cache, y: y - 8, angle: 360, alpha: { from: .72, to: 1 }, duration: 900, yoyo: true, repeat: -1, ease: 'Sine.inOut' });
    }
  ...
  const attach = scene => {
    if (!scene || scene.__gameplayMaxV2Attached) return;
    if (!scene.player || !scene.mission) return;
    scene.__gameplayMaxV2Attached = true;
    const data = {
      baseSpeed: scene.player.body?.maxVelocity?.x || BASE_SPEED,
      speedUntil: 0,
    ...
    if (gameEvents) {
      const onSignal = () => {
        ...
        data.signalWindow = now;
        data.signalCount += 1;
        if (data.signalCount >= 3) {
          data.signalCount = 0;
          data.bonusScore += 50;
          setSpeedBurst(scene, FLOW_SPEED, FLOW_MS, 'signal-flow');
          cue(scene, 'FLOW CHAIN · SPEED CHARGE', '#ffd06e');
        }
      };
      const onCheckpoint = () => {
        data.checkpointCount += 1;
        scene.energy = Math.min(scene.energyMax ?? scene.energy, (scene.energy || 0) + 8);
        cue(scene, 'CHECKPOINT SECURED · +8 ENERGY', '#aee37f');
      };
      gameEvents.on('signal', onSignal);
      gameEvents.on('checkpoint', onCheckpoint);
      scene.events.once('shutdown', () => {
        gameEvents.off('signal', onSignal);
        gameEvents.off('checkpoint', onCheckpoint);
      });
    }
  ...
  const findScene = () => {
    ...
    if (scene?.player && scene?.mission) attach(scene);
  };
  ...
  window.addEventListener('relay:mission-complete', event => {
    ...
    const data = scene && state.get(scene);
    if (!data) return;
  ...
  // Scene creation normally follows the core-ready event. This one-shot check
  // also covers cached/recovered launches without introducing polling.
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', findScene, { once: true });
  else queueMicrotask(findScene);
})();
