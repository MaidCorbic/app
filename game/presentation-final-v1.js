...
(() => {
  ...
  const hideLegacyGameplayHud = scene => {
    ...
    const height = Number(scene?.scale?.height || window.innerHeight || 720);
    for (const child of list) {
      if (!child || typeof child.text !== 'string') continue;
      if (keep.has(child)) continue;
      const fixed = child.scrollFactorX === 0 && child.scrollFactorY === 0;
      const bounds = child.getBounds?.();
      if (!fixed || !bounds || bounds.y > height * 0.88 && !isLegacyHudText(child.text)) continue;
      if (!isLegacyHudText(child.text)) continue;
  ...
  const layoutObjective = scene => {
    ...
    /* Keep the objective below the top HUD and out of the play center on both web and mobile. */
    const y = mobile ? Math.max(82, Math.min(112, h * .14)) : Math.max(92, Math.min(128, h * .14));
    const key = `${Math.round(w)}x${Math.round(h)}:${mobile}:${Math.round(targetW)}`;
