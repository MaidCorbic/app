...
export const RESPONSE_PROFILES = {
  CLEAN: { label: 'CITY CALM', accent: '#8df4ff', line: 'LOCAL RELAYS REMAIN OPEN', detail: 'Your last run kept the district stable.' },
  DAMAGED: { label: 'CITY ALERT', accent: '#ff9d6e', line: 'SECURITY PRESSURE DETECTED', detail: 'The district is reacting to the last delivery.' },
  NETWORKED: { label: 'CITY LINKED', accent: '#c8b5ff', line: 'RELAY RESPONSE PROPAGATED', detail: 'The network remembers your last intervention.' },
};
...
function showResponse(response, mode = 'visit') {
  ...
  node.__hideTimer = window.setTimeout(() => node.classList.remove('is-visible', 'is-burst', 'is-complete'), mode === 'visit' ? 1900 : 0);
}
...
function responseVisuals(scene, record) {
  ...
  if (response === 'DAMAGED') {
    ...
    alert?.lineStyle?.(2, color, .44); alert?.strokeRect?.(x - 18, y - 18, 36, 36);
    if (alert) { scene.tweens?.add?.({ targets: alert, alpha: { from: .18, to: .72 }, yoyo: true, repeat: 5, duration: 260 }); window.setTimeout(() => alert.destroy?.(), 3200); }
    scene.game?.events?.emit?.('city-response', { type: response, district: record.district }); return;
  }
  if (response === 'NETWORKED') {
    const beacon = scene.add?.circle?.(x, y, 14, color, .12)?.setStrokeStyle?.(2, color, .86)?.setDepth?.(25);
    if (beacon) scene.tweens?.add?.({ targets: beacon, scale: 3.2, alpha: 0, duration: 1200, repeat: 1, onComplete: () => beacon.destroy?.() });
    scene.game?.events?.emit?.('city-response', { type: response, district: record.district }); return;
  }
  const calm = scene.add?.circle?.(x, y, 9, color, .08)?.setStrokeStyle?.(2, color, .56)?.setDepth?.(25);
  if (calm) scene.tweens?.add?.({ targets: calm, scale: 2.1, alpha: 0, duration: 1000, onComplete: () => calm.destroy?.() });
...
function getPackageCondition(scene) { const value = scene?.packageCondition; return Number.isFinite(Number(value)) ? Number(value) : 100; }
...
function tick() { const scene = window.__relayRunnerScene; if (scene?.mission?.id) bindScene(scene); }
