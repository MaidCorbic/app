...
function startSlideJump(scene, state) {
  ...
  const direction = Math.sign(vx || player.scaleX || 1) || 1;
  ...
  if (typeof body.setVelocityX === 'function') body.setVelocityX(horizontal);
  else body.velocity.x = horizontal;
  if (typeof body.setVelocityY === 'function') body.setVelocityY(-BASE_SLIDE_JUMP * bonus);
  else body.velocity.y = -BASE_SLIDE_JUMP * bonus;
  ...
  scene.game?.events?.emit('slide-jump', {
    perfect,
...
function update(scene) {
  ...
  if (!sliding && state.wasSliding && !state.slideJumped) {
    state.chain = 0;
  }
  ...
  if (!grounded(scene) && !sliding) {
    const target = (body.velocity?.x || 0) + ((state.jumpHeld ? 1 : -1) * AIR_CONTROL);
    if (typeof body.setVelocityX === 'function') body.setVelocityX(Math.max(-MAX_MOMENTUM, Math.min(MAX_MOMENTUM, target)));
  }
...
if (!RunnerScene.prototype.__slideJumpMomentumV1UpdatePatched) {
  RunnerScene.prototype.update = function slideJumpMomentumUpdate(...args) {
