(() => {
  ...
  root.innerHTML = `<div class="tutorial-map" aria-hidden="true"><div class="tutorial-map-head"><span>TRAINING ROUTE</span><b>01 / 05</b></div><div class="tutorial-route"><i></i><span data-step="0"></span><span data-step="1"></span><span data-step="2"></span><span data-step="3"></span><span data-step="4"></span></div></div><div class="tutorial-card"><p class="tutorial-kicker">FIRST DELIVERY · OLD QUARTER</p><h2></h2><p class="tutorial-copy"></p><div class="tutorial-key"></div><div class="tutorial-progress"></div></div><div class="tutorial-cinema" hidden><div class="tutorial-cinema-shell"><p>TRAINING COMPLETE</p><div class="tutorial-cinema-line"></div><h2>THE CITY<br><em>IS LIVE.</em></h2><span>Orientation link closed. Your first real delivery is now active.</span><small>FOLLOW THE GOLD SIGNALS · DELIVERY CONTROL TRANSFERRED</small></div></div>`;
  document.body.appendChild(root);

  const style = document.createElement('style');
  style.textContent = `
    #relayTutorialOnboardingV2{position:fixed;inset:0;z-index:2147483000;pointer-events:none;color:#eafcff;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
    #relayTutorialOnboardingV2[hidden],#relayTutorialOnboardingV2 .tutorial-map[hidden],#relayTutorialOnboardingV2 .tutorial-card[hidden],#relayTutorialOnboardingV2 .tutorial-cinema[hidden]{display:none!important}
    ...
    #relayTutorialOnboardingV2 .tutorial-cinema{position:absolute;inset:0;display:grid;place-items:center;padding:clamp(18px,4vw,48px);background:radial-gradient(circle at 50% 42%,rgba(19,126,154,.2),rgba(1,5,12,.97) 62%);backdrop-filter:blur(12px);text-align:center;pointer-events:auto}.tutorial-cinema-shell{width:min(760px,92vw);padding:clamp(28px,5vw,54px);border:1px solid rgba(98,239,255,.28);border-radius:26px;background:linear-gradient(155deg,rgba(7,25,42,.88),rgba(2,8,17,.97));box-shadow:0 35px 120px rgba(0,0,0,.65),0 0 70px rgba(72,219,255,.12),inset 0 0 45px rgba(96,218,255,.025)}.tutorial-cinema-shell p{margin:0;color:#70efff;font-size:10px;font-weight:900;letter-spacing:.3em}.tutorial-cinema-line{width:110px;height:2px;margin:18px auto 22px;background:linear-gradient(90deg,transparent,#74efff,transparent);box-shadow:0 0 16px rgba(116,239,255,.7)}.tutorial-cinema-shell h2{margin:0;color:#f4fbff;font-size:clamp(42px,7vw,84px);line-height:.9;letter-spacing:-.055em}.tutorial-cinema-shell h2 em{font-style:normal;color:#ffd36f;text-shadow:0 0 28px rgba(255,211,111,.18)}.tutorial-cinema-shell span{display:block;margin:24px auto 0;max-width:610px;color:#c9d8e5;font-size:clamp(12px,1.5vw,16px);line-height:1.6}.tutorial-cinema-shell small{display:block;margin-top:22px;color:#82efff;font-size:9px;font-weight:900;letter-spacing:.18em;opacity:.8}@keyframes tutorialCinema{from{opacity:0;transform:scale(1.015)}to{opacity:1;transform:scale(1)}}
    body.relay-cinematic-active #relayTutorialOnboardingV2{pointer-events:auto}
    body.relay-cinematic-active #game .hud,body.relay-cinematic-active #game .world-marker,body.relay-cinematic-active #game .input-guide,body.relay-cinematic-active #game .mobile-controls,body.relay-cinematic-active #game .rotate-prompt,body.relay-cinematic-active #game #pause,body.relay-cinematic-active #game #toast,body.relay-cinematic-active #pauseMenu,body.relay-cinematic-active #finish,body.relay-cinematic-active #gameOver,body.relay-cinematic-active #levelUp,body.relay-cinematic-active #abilityUnlock{display:none!important;visibility:hidden!important;pointer-events:none!important}
    @media(max-width:700px){#relayTutorialOnboardingV2 .tutorial-map{top:max(10px,env(safe-area-inset-top));left:50%;transform:translateX(-50%);width:min(92vw,390px)}#relayTutorialOnboardingV2 .tutorial-card{bottom:max(18px,env(safe-area-inset-bottom));padding:16px 14px;width:min(92vw,640px)}.tutorial-copy{font-size:12px}.tutorial-cinema-shell{padding:26px 18px;border-radius:20px}.tutorial-cinema-shell h2{font-size:clamp(40px,12vw,64px)}.tutorial-cinema-shell span{font-size:12px}.tutorial-cinema-shell small{font-size:8px;line-height:1.5}}
  `;
  ...
  const showStep = () => { const current = steps[state.step]; if (!current) return; root.hidden=false; root.classList.add('active'); root.classList.remove('cinematic'); el('.tutorial-card').hidden=false; el('.tutorial-map').hidden=false; el('.tutorial-cinema').hidden=true; el('.tutorial-card h2').textContent=current.title; el('.tutorial-copy').textContent=current.text; el('.tutorial-key').textContent=current.key; el('.tutorial-map-head b').textContent=`${String(state.step+1).padStart(2,'0')} / 05`; root.querySelectorAll('.tutorial-route span').forEach((node,index)=>node.className=index<state.step?'done':index===state.step?'active':''); el('.tutorial-progress').innerHTML=steps.map((_,index)=>`<i class="${index<state.step?'done':index===state.step?'active':''}"></i>`).join(''); persist(); };
  ...
  const setCinematicHud = enabled => {
    document.body.classList.toggle('relay-cinematic-active', enabled);
    if (enabled) {
      root.classList.add('cinematic');
      root.querySelector('.tutorial-card').hidden = true;
      root.querySelector('.tutorial-map').hidden = true;
    } else {
  ...
  const cinematic = () => {
    ...
    let unlocked=false;
    const unlock = () => { if(unlocked) return; unlocked=true; window.dispatchEvent(new Event('relay:cinematic-unlock')); cinema.hidden=true; root.hidden=true; setCinematicHud(false); state.completed=true; sessionStorage.setItem(COMPLETE_KEY,'1'); window.dispatchEvent(new CustomEvent('relay:tutorial-complete',{detail:{scene:state.scene}})); };
