...
const openInfoPanel = kind => {
  ...
  if (kind === 'faq') {
    ...
    content.innerHTML = `<div class="relay-faq-list">${RELAY_FAQ.map(([question,answer]) => `<article class="relay-faq-item"><button class="relay-faq-question" type="button">${question}</button><div class="relay-faq-answer">${answer}</div></article>`).join('')}</div>`;
  } else {
    eyebrow.textContent = LATEST_UPDATE.version;
    heading.textContent = LATEST_UPDATE.title;
    content.innerHTML = `<p class="relay-update-meta">LATEST UPDATE</p><div class="relay-update-list">${LATEST_UPDATE.items.map(item => `<div class="relay-update-item">${item}</div>`).join('')}</div>`;
  }
};

const installPanelBehavior = () => {
  document.querySelectorAll('[data-relay-info]').forEach(button => {
    if (button.dataset.relayInfoBound === 'true') return;
    button.dataset.relayInfoBound = 'true';
    button.addEventListener('click', () => openInfoPanel(button.dataset.relayInfo));
  });
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => { installPanelBehavior(); installExit(); }, { once:true });
else { installPanelBehavior(); installExit(); }
...
import './relay-senior-polish-v2.js';