...
// P1 GAMEPLAY CORRECTNESS V2
// Keeps authoritative gameplay integrations without frame-driven HUD polling.
...
(() => {
  ...
  const patchIntel = () => {
    ...
    if (typeof originalShow === 'function') RunnerScene.prototype.showEnemyDiscovery = function (...args) {
      const result = originalShow.apply(this, args);
      if (this.__enemyDiscoveryActiveKey) freeze(this, 'enemy-intel');
      return result;
    };
    if (typeof originalDismiss === 'function') RunnerScene.prototype.dismissEnemyDiscovery = function (...args) {
      const result = originalDismiss.apply(this, args); thaw(this, 'enemy-intel'); return result;
    };
    if (typeof originalIntelDismiss === 'function') RunnerScene.prototype.dismissIntelCard = function (...args) {
      const result = originalIntelDismiss.apply(this, args); thaw(this, 'enemy-intel'); return result;
    };
  ...
  const installDashIntegration = () => {
    ...
    RunnerScene.prototype.update = RunnerScene.prototype.update;
  ...
  const installGameplayChain = () => {
    ...
    const getState = scene => state.get(scene) || (() => { const s = { chain: 0, last: '', lastAt: 0 }; state.set(scene, s); return s; })();
    const bind = scene => {
      ...
      scene.game.events.on('dash-start', () => { s.chain += 1; s.last = 'dash'; s.lastAt = performance.now(); });
      scene.events.once('shutdown', () => scene.game.events.off('feedback', onFeedback));
    };
