...
(() => {
  ...
  const start = () => {
    ...
    const ui=splash.querySelector('.relay-splash-ui'); const label=ui?.querySelector('.relay-splash-status'); const percent=ui?.querySelector('.relay-splash-percent'); const bar=ui?.querySelector('.relay-splash-progress');
    ...
    const copy=document.createElement('div'); copy.className='arrival-copy'; copy.innerHTML='<div class="arrival-center"><p class="arrival-kicker">CHAPTER 01 // NIGHT SHIFT</p><h1 class="arrival-title"><span>THE NIGHT</span><br><em>IS ONLINE.</em></h1><div class="arrival-line"></div><p class="arrival-message">THE CITY IS SLEEPING. THE NETWORK IS NOT.<br>ONE RUNNER. ONE SIGNAL. NO SECOND CHANCE.</p><div class="arrival-mission"><small>MISSION 01 // OLD QUARTER</small><b>ROOFTOP RELAY</b><span>FOLLOW THE RELAY · RESTORE THE SIGNAL</span></div></div>';
    splash.append(status,signal,particles,copy);
    const image=splash.querySelector('#relaySplashArt,.relay-splash-art');
    const started=performance.now(); const MIN_MS=1200; let released=false; let imageReady=!!(image?.complete&&image?.naturalWidth); let domReady=document.readyState!=='loading'; let pageReady=document.readyState==='complete'; let engineReady=!!document.querySelector('#phaser-game canvas'); let progress=0;
    ...
    const tick=()=>{if(released)return;const target=stateProgress();if(target>progress){const from=progress,to=target,t0=performance.now();const frame=now=>{const t=Math.min(1,(now-t0)/500);setProgress(from+(to-from)*(t*(2-t)));if(t<1&&!released)requestAnimationFrame(frame)};requestAnimationFrame(frame)};if(engineReady&&imageReady&&domReady&&pageReady&&performance.now()-started>=MIN_MS)release('ready');else if(!released)window.setTimeout(tick,50)};
    const release=reason=>{if(released||!document.body.contains(splash))return;released=true;setProgress(100);splash.dataset.cinematicReleased='true';splash.classList.add('is-leaving');splash.setAttribute('aria-busy','false');window.setTimeout(()=>splash.remove(),450);window.dispatchEvent(new CustomEvent('relay:splash-released',{detail:{reason}}))};
    ...
    window.setTimeout(()=>{if(!released&&imageReady&&domReady)release('failsafe')},6500);
