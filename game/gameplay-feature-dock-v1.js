...
(() => {
  ...
  const installStyle = () => {
    ...
    style.textContent = `
      #relay-feature-dock-v1{position:absolute;right:16px;top:96px;z-index:275;display:grid;grid-template-columns:minmax(0,150px);gap:7px;pointer-events:none}
      ...
      @media(max-width:700px){#relay-feature-dock-v1{right:8px;top:84px;width:min(150px,calc(100vw - 16px));gap:5px}#relay-feature-dock-v1 .fd-card{padding:7px 8px}.fd-sub{font-size:5.5px}.fd-actions{margin-top:6px}.fd-actions button{min-height:29px;font-size:6.2px}}
      @media(max-width:430px){#relay-feature-dock-v1{top:78px;width:min(145px,calc(100vw - 14px))}#relay-feature-dock-v1 .fd-card{padding:6px 7px}.fd-title{font-size:6.5px}.fd-state{font-size:5px}.fd-sub{font-size:5px}}
    `;
    document.head.appendChild(style);
  };

  const synthKey = code => {
    const event = new KeyboardEvent('keydown', { key: 'g', code, bubbles: true, cancelable: true });
  ...
  const mount = play => {
    ...
    root.hidden = true;
    root.innerHTML = '<div class="fd-card" data-feature="grapple"><div class="fd-title"><span>GRAPPLE</span><b class="fd-state">READY</b></div><div class="fd-sub">ANCHOR TRAVERSAL · G</div><div class="fd-actions"><button type="button" data-feature-action="grapple">LINK</button></div></div>';
    play.appendChild(root);
    root.addEventListener('pointerdown', event => {
      const button = event.target.closest?.('[data-feature-action="grapple"]');
      if (!button) return;
  ...
  if (!RunnerScene.prototype.__relayFeatureDockV2Update) {
    RunnerScene.prototype.update = function featureDockUpdate(time, delta, ...args) {
      ...
      try {
        ...
        console.warn('[Relay Runner] feature dock skipped:', error);
      }
      return result;
    };
