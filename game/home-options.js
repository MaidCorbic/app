...
(() => {
  ...
  style.textContent = `
    ...
    #titlePanel.home-options-exclusive #titlePanelContent{box-sizing:border-box!important;min-height:0!important;width:100%!important;max-height:calc(100dvh - 175px)!important;max-height:calc(100svh - 175px)!important;overflow-x:hidden!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior:contain!important;touch-action:pan-y!important;scrollbar-width:thin!important;scrollbar-color:rgba(255,208,110,.78) rgba(7,17,31,.72)!important}
    #titlePanel.home-options-exclusive #titlePanelContent::-webkit-scrollbar{width:7px;height:7px}
    #titlePanel.home-options-exclusive #titlePanelContent::-webkit-scrollbar-track{background:linear-gradient(180deg,rgba(7,17,31,.82),rgba(10,22,38,.72));border-left:1px solid rgba(255,255,255,.035);border-radius:999px}
    #titlePanel.home-options-exclusive #titlePanelContent::-webkit-scrollbar-thumb{background:linear-gradient(180deg,rgba(255,208,110,.92),rgba(255,174,74,.78));border:1px solid rgba(5,12,23,.72);border-radius:999px;box-shadow:0 0 10px rgba(255,208,110,.18)}
    #titlePanel.home-options-exclusive #titlePanelContent::-webkit-scrollbar-thumb:hover{background:linear-gradient(180deg,#ffe08b,#ffc05c);box-shadow:0 0 14px rgba(255,208,110,.28)}
    ...
    @media(max-width:700px){
      ...
      #titlePanel.home-options-exclusive #titlePanelContent::-webkit-scrollbar{width:6px}
  ...
  `;
  const styleMount = document.head.appendChild(style);
  ...
  let rendering = false;
  function isOptionsPanelOpen() { const panel=document.getElementById('titlePanel'); const heading=document.getElementById('titlePanelHeading'); if(!panel||panel.classList.contains('hidden'))return false; const title=heading?.textContent.trim().toUpperCase()||''; return title.includes('RUN SETTINGS')||title==='OPTIONS'; }
  function render(){ if(rendering||!isOptionsPanelOpen())return; const content=document.getElementById('titlePanelContent'); const heading=document.getElementById('titlePanelHeading'); const panel=document.getElementById('titlePanel'); if(!content||!heading||!panel)return; rendering=true; panel.classList.add('home-options-exclusive'); heading.textContent='OPTIONS'; const eyebrow=document.getElementById('titlePanelEyebrow'); if(eyebrow)eyebrow.textContent='RELAY RUNNER // SYSTEM SETTINGS'; content.replaceChildren(); const s=getState(); const lang=LANGUAGES.find(x=>x[0]===language())||LANGUAGES[0]; content.innerHTML=`<div class="home-options-v3"><div class="home-section">GAMEPLAY</div><div class="home-section">GUIDANCE</div>${optionButton('TUTORIAL','tutorialEnabled',s.tutorialEnabled!==false,'Mission guidance and contextual lessons')}${optionButton('GAME AUDIO','muted',!s.muted,'Master game audio')}${optionButton('SCREEN SHAKE','screenShake',!!s.screenShake,'Camera impact feedback')}${optionButton('REDUCED MOTION','reducedMotion',!!s.reducedMotion,'Reduce movement effects')}${optionButton('ATMOSPHERIC RAIN','rain',!!s.rain,'City weather ambience')}<div class="home-section">VOICE & AUDIO</div>${optionButton('AI VOICE','aiVoice',s.aiVoice!==false,'NIA / MARA spoken game guidance')}<label class="home-opt"><span class="home-opt-copy"><b>MUSIC</b><small><span data-music-value>${Math.round((s.musicVolume??.55)*100)}%</span> VOLUME</small></span><input data-home-volume="musicVolume" type="range" min="0" max="1" step=".05" value="${s.musicVolume??.55}"></label><label class="home-opt"><span class="home-opt-copy"><b>SFX</b><small><span data-sfx-value>${Math.round((s.sfxVolume??.7)*100)}%</span> VOLUME</small></span><input data-home-volume="sfxVolume" type="range" min="0" max="1" step=".05" value="${s.sfxVolume??.7}"></label><div class="home-section">LANGUAGE</div><div class="home-opt home-lang"><div class="home-opt-copy"><b>GAME LANGUAGE</b><small>Choose your interface language</small></div><button type="button" data-home-language>🌐 ${lang[1]}</button><div class="home-lang-menu hidden" data-home-language-menu>${LANGUAGES.map(([code,name])=>`<button type="button" data-language="${code}" class="${code===lang[0]?'active':''}">${name}</button>`).join('')}</div></div><div class="home-section">DISPLAY</div><div class="home-options-actions"><button type="button" data-home-fullscreen>FULLSCREEN</button><button type="button" data-home-reset>RESET OPTIONS</button></div><div class="home-section">CONTROLS</div><div class="home-opt home-controls"><small>A / D MOVE · SPACE JUMP · E FIRE · Q BLADE · SHIFT DASH</small></div></div>`;
    ...
    const langButton=content.querySelector('[data-home-language]');const langMenu=content.querySelector('[data-home-language-menu]');langButton?.addEventListener('click',e=>{e.stopPropagation();langMenu?.classList.toggle('hidden');});content.querySelectorAll('[data-language]').forEach(btn=>btn.addEventListener('click',()=>{setLanguage(btn.dataset.language);render();}));content.querySelector('[data-home-fullscreen]')?.addEventListener('click',async()=>{try{if(!document.fullscreenElement)await document.documentElement.requestFullscreen?.();else await document.exitFullscreen?.();}catch{}});content.querySelector('[data-home-reset]')?.addEventListener('click',()=>{savePatch({muted:false,musicVolume:.55,sfxVolume:.7,screenShake:true,reducedMotion:false,rain:true,aiVoice:true,tutorialEnabled:true});localStorage.removeItem(LANGUAGE_KEY);setLanguage('en');window.dispatchEvent(new CustomEvent('relay-settings-change',{detail:{reset:true}}));render();});rendering=false;}
  const openOptions=()=>{const panel=document.getElementById('titlePanel');if(panel)panel.classList.add('home-options-exclusive');window.setTimeout(render,0);window.setTimeout(render,30);};
  const init=()=>{const panel=document.getElementById('titlePanel');if(!panel)return;document.querySelectorAll('[data-title-panel="controls"]').forEach(button=>button.addEventListener('click',openOptions));const observer=new MutationObserver(mutations=>{if(mutations.some(m=>m.type==='attributes'&&m.attributeName==='class')){if(!panel.classList.contains('hidden'))openOptions();}const content=document.getElementById('titlePanelContent');if(content&&mutations.some(m=>m.type==='childList'&&m.target===content)&&!content.querySelector('.home-options-v3'))openOptions();});observer.observe(panel,{attributes:true,attributeFilter:['class'],childList:true,subtree:true});document.addEventListener('click',e=>{if(!e.target.closest('.home-lang'))document.querySelectorAll('.home-lang-menu').forEach(menu=>menu.classList.add('hidden'));});};
