...
(() => {
  ...
  const openInfoPanel = kind => {
    ...
    if (kind === 'faq') {
      ...
      eyebrow.textContent = LATEST_UPDATE.version || latestUpdates[0].version;
      heading.textContent = 'UPDATE';
      content.innerHTML = '<p class="relay-update-meta">LIVE CHANNEL · CHAPTER 01 / NIGHT SHIFT</p><div class="relay-update-list">' + readUpdates().map(item => `<div class="relay-update-item"><b>${item.title || 'SYSTEM UPDATE'}</b><span>${item.version || 'LIVE'} · ${item.date || ''}</span><small>${item.detail || ''}</small></div>`).join('') + '</div><div class="relay-update-actions"><button type="button" class="relay-ui-button primary" data-relay-home-refresh>REFRESH NOW</button></div>';
      content.querySelector('[data-relay-home-refresh]')?.addEventListener('click', refreshUpdates);
    }
  };
  ...
  const install = () => {
    ...
    new MutationObserver(injectHomeLinks).observe($('intro') || document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','hidden']});
  };
