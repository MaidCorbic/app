...
(() => {
  ...
  const readUpdates = () => {
    ...
    } catch { return latestUpdates.slice(); }
  };
  ...
  const announce = (text) => {
    ...
    announce._timer = window.setTimeout(() => toast.classList.remove('show'), 1800);
  };

  const injectHomeUpdate = () => {
    const launcher = document.querySelector('#intro .info-launcher');
    if (!launcher) return;
    const oldFaq = launcher.querySelector('[data-relay-info="faq"]');
    const oldInfo = launcher.querySelector('[data-relay-info="update"]');
    oldFaq?.remove();
    oldInfo?.remove();
    if (launcher.querySelector('[data-relay-update-open]')) return;
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'relay-home-update';
    button.dataset.relayUpdateOpen = '1';
  ...
  const refreshUpdates = () => {
    ...
    if (Array.isArray(custom) && custom.length) writeUpdates(custom);
    else writeUpdates(latestUpdates);
    renderUpdateCenter();
  ...
  const bindUpdateButton = () => {
    ...
    button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); renderUpdateCenter(); });
  };
  ...
  const addLiveUpdate = update => {
    ...
    const current = readUpdates().filter(item => item.id !== incoming.id);
    writeUpdates([incoming, ...current]);
    const host = $('relayUpdateCenter');
    if (host && !host.classList.contains('hidden')) renderUpdateCenter();
  ...
  const ensureGameplayElements = () => {
    ...
    if (!intel) {
      intel = document.createElement('section');
    ...
    if (!rotate) {
      rotate = document.createElement('section');
      rotate.id = 'relayRotateCard';
      rotate.className = 'relay-rotate-card';
      rotate.setAttribute('role','status');
      rotate.innerHTML = '<div class="rotate-icon" aria-hidden="true">↻</div><h3>ROTATE YOUR DEVICE</h3><p>LANDSCAPE MODE RECOMMENDED<br>More space. Better control. Better run.</p><button type="button" data-relay-rotate-dismiss>CONTINUE</button>';
      document.body.append(rotate);
      rotate.querySelector('[data-relay-rotate-dismiss]')?.addEventListener('click', () => { document.body.classList.add('rotate-dismissed'); rotate.classList.remove('is-visible'); });
    }
  };

  const getMission = () => {
    const number = Number(($('missionNumber')?.textContent || '').replace(/\D/g,''));
    if (Number.isFinite(number) && number >= 1 && missions[number-1]) return missions[number-1];
    const title = ($('objective')?.textContent || '').trim().toLowerCase();
    return missions.find(mission => mission.title.toLowerCase() === title) || missions[0];
  };
  ...
  const missionIntel = mission => {
    ...
    return map[id] || ['MISSION INTELLIGENCE','Stay focused on the current objective and read the route cues.', [`LEVEL ${missions.indexOf(mission)+1}`,'LIVE']];
  };

  let intelTimer;
  const showIntel = (reason = 'mission') => {
    ensureGameplayElements();
    const intel = $('relayGameplayIntel');
    const mission = getMission();
    if (!intel || !mission) return;
    ...
    intel.querySelector('.intel-meta').innerHTML = meta.map(item => `<span class="intel-pill">${item}</span>`).join('') + `<span class="intel-pill">${String(reason).toUpperCase()}</span>`;
    intel.classList.add('is-active');
    window.clearTimeout(intelTimer);
    intelTimer = window.setTimeout(() => intel.classList.remove('is-active'), 4200);
  };

  const syncRotateCard = () => {
    ensureGameplayElements();
    const rotate = $('relayRotateCard');
    const play = $('play');
    if (!rotate || !play) return;
    const gameplayVisible = !play.classList.contains('hidden') && !$('intro')?.classList.contains('hidden') === false && !$('finish')?.classList.contains('hidden') && !$('gameOver')?.classList.contains('hidden');
    const portrait = window.matchMedia('(orientation: portrait)').matches;
    const mobile = document.body.classList.contains('is-touch') || window.innerWidth <= 760;
    const shouldShow = mobile && portrait && gameplayVisible && !document.body.classList.contains('rotate-dismissed');
    rotate.classList.toggle('is-visible', shouldShow);
  };
  ...
  const bindGameplayObservers = () => {
    ...
    const routeIntel = $('routeIntel');
    [missionNumber, objective, routeIntel].filter(Boolean).forEach(node => {
      new MutationObserver(() => { showIntel('route update'); syncRotateCard(); }).observe(node, { childList:true, characterData:true, subtree:true });
    });
    ...
    window.addEventListener('gameplay:v12:event', event => { const type = event.detail?.type || 'event'; showIntel(type); });
  ...
  const install = () => {
    ...
    link.rel = 'stylesheet';
    link.href = './unified-gameplay-ui-v1.css';
    if (!document.querySelector('link[href="./unified-gameplay-ui-v1.css"]')) document.head.append(link);

    const updateHost = document.createElement('section');
    updateHost.id = 'relayUpdateCenter';
    updateHost.className = 'relay-update-center hidden';
    updateHost.setAttribute('aria-label','Live updates');
    document.body.append(updateHost);

    bindUpdateButton();
    bindGameplayObservers();
    syncRotateCard();
    ...
    window.addEventListener('storage', event => { if (event.key === UPDATE_KEY) { if ($('relayUpdateCenter') && !$('relayUpdateCenter').classList.contains('hidden')) renderUpdateCenter(); } });
