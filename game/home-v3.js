...
(() => {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .home-v3-play .home-v3-play-knob::after{
        ...
        border-radius:15px;
        animation:homeV3PlayPulse 1.8s ease-in-out infinite
      }
      ...
      #titlePanelContent [data-unified-toggle="tutorialEnabled"],
      #titlePanelContent .relay-option-card:has(
  ...
  const removeTutorialSurface = () => {
    ...
    document
      ...
      .forEach(node =>
        node.closest('.relay-option-card')?.remove()
      );
  };
  ...
  const installSwipePlay = button => {
    ...
    button.innerHTML =
      '<span class="home-v3-play-track" aria-hidden="true"></span>' +
      '<span class="home-v3-play-fill" aria-hidden="true"></span>' +
      '<span class="home-v3-play-label">PLAY NOW</span>' +
      '<span class="home-v3-play-hint">SWIPE TO DEPLOY →</span>' +
      '<span class="home-v3-play-knob" aria-hidden="true">→</span>';
    ...
    const updateMetrics = () => {
      ...
      const knobWidth =
        knob?.getBoundingClientRect().width || 42;

      maxTravel =
        Math.max(
          1,
          rect.width - knobWidth - 14
        );
  ...
  const build = () => {
    ...
    shell.innerHTML = `
      ...
      <main class="home-v3-main">
        <section>
          ...
          <p class="home-v3-copy">
            ...
            than anyone else can. Build your route, master
            the night and keep the line open.
          </p>
      ...
      <footer class="home-v3-footer">
        <span>
          RELAY RUNNER · <b>VERSION 1.1.0</b>
        </span>

        <span>
          W / D MOVE · SPACE JUMP · ESC PAUSE
        </span>
      </footer>
    `;

    /*
     * Home V3 owns only the presentation shell.
     *
     * IMPORTANT:
     * Do not move the legacy Home menu or info launcher
     * back into #intro here. The final Home layout owns
     * the menu separately.
     */
    ...
    const syncContinue = () => {
      ...
      button.hidden =
        legacy.classList.contains('hidden') ||
