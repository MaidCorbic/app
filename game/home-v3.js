...
(() => {
  ...
  /*
   * Trigger original game buttons.
   * IMPORTANT:
   * #start and #continue remain in the DOM because
   * main.js owns the actual gameplay logic.
   */
  const nativeClick = selector => {
    ...
    if (!target) {
      console.warn(
        `[Home V3] Target not found: ${selector}`
      );
      return false;
    }
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      /* ========================================================
         ...
         DO NOT REMOVE THIS FROM THE DOM.
         
         main.js may own #start / #continue inside it.
         We only hide the old visual menu.
         ======================================================== */
      ...
      /*
       * Explicitly hide old duplicate HOME controls.
       */
  ...
  const installSwipePlay = button => {
    ...
    const complete = () => {
      ...
      /*
       * IMPORTANT:
       * The original #start button is still
       * present in the DOM but hidden visually.
       */
      window.setTimeout(() => {
        const started =
          nativeClick('#start');
        ...
        if (!started) {
          console.error(
            '[Home V3] #start button was not found.'
          );
        }
      }, 180);
    };
  ...
  const build = () => {
     ...
     * IMPORTANT:
     *
     * We keep these references.
     *
     * The legacy menu may contain:
     *   #start
     *   #continue
     *
     * Those are required by main.js.
     */
    const legacyMenu =
      intro.querySelector(
        '.main-menu'
    ...
    shell.innerHTML = `
      ...
      >
        ...
        <section>
          ...
          <p
            class="home-v3-kicker"
          >
            ROOFTOP DELIVERY NETWORK
            · CHAPTER 01
          </p>
     ...
     * IMPORTANT:
     *
     * Keep legacyMenu and launcher in the DOM.
     *
     * CSS hides them.
     *
     * This preserves #start/#continue for main.js.
     * ----------------------------------------------------------
     */
  ...
  const start = () => {
    ...
    /*
     * Watch HOME state and tutorial changes.
     */
    const observer =
      new MutationObserver(() => {
        syncSurface();
