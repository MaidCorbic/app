...
(() => {
  ...
  let ctx = null, master = null, unlocked = false, musicTimer = null, musicActive = false;
  const seen = new WeakSet(), lastShot = new Map();
  const AC = window.AudioContext || window.webkitAudioContext;
  if (!AC) return;

  const ensure = async () => {
    if (!ctx) { ctx = new AC(); master = ctx.createGain(); master.gain.value = .20; master.connect(ctx.destination); }
    if (ctx.state === 'suspended') await ctx.resume().catch(() => {});
    unlocked = ctx.state === 'running'; return unlocked;
  };

  const tone = (f, start, duration, volume = .08, type = 'sine', endF = f) => {
    if (!ctx || !master || !unlocked) return;
    const osc = ctx.createOscillator(), gain = ctx.createGain();
    osc.type = type; osc.frequency.setValueAtTime(Math.max(20, f), start); osc.frequency.exponentialRampToValueAtTime(Math.max(20, endF), start + duration);
    gain.gain.setValueAtTime(.0001, start); gain.gain.exponentialRampToValueAtTime(Math.max(.0001, volume), start + .008); gain.gain.exponentialRampToValueAtTime(.0001, start + duration);
    osc.connect(gain).connect(master); osc.start(start); osc.stop(start + duration + .025);
  };
  ...
  const noise = (start, duration = .07, volume = .045, high = 1800) => {
    ...
    filter.type = 'highpass'; filter.frequency.value = high; gain.gain.setValueAtTime(volume, start); gain.gain.exponentialRampToValueAtTime(.0001, start + duration);
  ...
  const play = kind => {
    ...
    if (kind === 'signal') return (tone(660, t, .08, .08), tone(990, t + .055, .12, .065));
    ...
    if (kind === 'mission-complete') return (tone(440, t, .11, .065), tone(660, t + .09, .12, .07), tone(880, t + .19, .2, .065));
    if (kind === 'gun') return (noise(t, .028, .08, 1050), tone(145, t, .075, .09, 'triangle', 72));
    if (kind === 'egg') return (tone(470, t, .055, .065, 'triangle', 320), noise(t + .04, .085, .075, 1350));
    if (kind === 'dino') return (tone(125, t, .22, .08, 'sawtooth', 70), noise(t + .03, .1, .03, 500));
    if (kind === 'invader') return (tone(880, t, .11, .075, 'sine', 360), tone(1320, t + .02, .055, .04, 'triangle', 620));
    if (kind === 'alien') return (tone(245, t, .15, .075, 'triangle', 105), noise(t, .055, .03, 1000));
    if (kind === 'boss') return (tone(82, t, .25, .095, 'sine', 42), noise(t, .09, .045, 450));
  };
  ...
  const gesture = () => ensure().then(() => { if (introVisible()) startMusic(); else stopMusic(); });
  ...
  if (intro) new MutationObserver(() => { if (!introVisible()) stopMusic(); }).observe(intro, { attributes: true, attributeFilter: ['class'] });
  ...
  document.addEventListener('pointerdown', event => { if (event.target.closest?.(buttonSelectors)) ensure().then(() => play('ui')); }, { passive: true, capture: true });
  ...
  const classify = obj => {
    ...
    if (/egg/.test(key)) return 'egg'; if (/dino/.test(key)) return 'dino'; if (/invader|sentinel|storm/.test(key)) return 'invader'; if (/alien|plasma/.test(key)) return 'alien'; if (/boss|titan|apex/.test(key)) return 'boss'; if (/bullet|shot|projectile|bolt|laser|plasma|fire|comet|kinetic/.test(key)) return 'gun'; return null;
  };
  ...
  const announce = (scene, before) => {
    ...
    for (const obj of scene.children?.list || []) {
      ...
      const direct = classify(obj); if (!direct) continue; seen.add(obj);
      let sound = direct;
      if (direct === 'gun') {
        let nearest = null, best = Infinity;
        for (const enemy of enemies) { if (!enemy?.active) continue; const d = Math.hypot((enemy.x || 0) - (obj.x || 0), (enemy.y || 0) - (obj.y || 0)); if (d < best) { best = d; nearest = enemy; } }
        if (nearest && best < 260) sound = classifyEnemy(nearest);
      }
      const now = performance.now(); if (now - (lastShot.get(sound) || 0) > 110) { lastShot.set(sound, now); ensure().then(() => play(sound)); }
    }
  };
  const wrap = name => {
    ...
    RunnerScene.prototype[name] = function trackedAudioUpdate(...args) { const before = new Set(this.children?.list || []); const result = original.apply(this, args); announce(this, before); return result; };
    RunnerScene.prototype[`__relayAudioWrapped_${name}`] = true;
  };
  wrap('updateEnemies'); wrap('updateSciFiThreats');
