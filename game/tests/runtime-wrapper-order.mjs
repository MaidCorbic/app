...
// Mobile input has one explicit module entry in index.html. relay-ui-init.js is
// deliberately CSS/runtime bootstrap only and must not become a second input owner.
assert.equal((index.match(/src\/systems\/mobile-input-single-owner-v1\.js/g) || []).length, 1);
assert.equal((main.match(/mobile-input-single-owner-v1/g) || []).length, 1);
