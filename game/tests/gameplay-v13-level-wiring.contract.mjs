...
const systemBlock=systems.match(/const SYSTEMS=\[(.*?)\];/s)?.[1] || '';
const ids=[...systemBlock.matchAll(/\['([A-Z0-9]+)','[^']+'\]/g)].map(m=>m[1]);
...
assert.match(src,/setScrollFactor\(0\)/,'V13 status UI must be camera-fixed for mobile and desktop visibility');
