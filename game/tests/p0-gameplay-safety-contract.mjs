...
assert.match(stability, /if \(this\.respawning \|\| this\.finished/);
...
// Main currently ships the V9 mobile single-owner contract.
...
// Dash must preserve a newer gravity value instead of restoring an obsolete one.
assert.match(dash, /Only restore the value captured at dash start if no other system has/);
assert.match(dash, /Number\(body\.gravity\?\.y\)===0|Number\(body\.gravity\?\.y\) === 0/);
