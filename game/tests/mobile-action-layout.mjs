...
const css = await readFile(fileURLToPath(new URL('mobile-final-polish.css', root)), 'utf8');
...
assert.match(css, /grid-template-columns:repeat\(3,var\(--touch-size\)\)/);
assert.match(css, /grid-template-rows:repeat\(2,var\(--touch-size\)\)/);
assert.match(css, /env\(safe-area-inset-bottom/);
assert.match(css, /-webkit-overflow-scrolling:touch/);
assert.match(index, /data-mobile-action=\"fire\"/);
assert.equal((index.match(/data-mobile-action=/g) || []).length, 6);

console.log('Mobile action layout contract V2: PASS');
