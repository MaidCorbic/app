...
assert.equal(
  packageJson.scripts['test:final-stability']?.length > 0,
  true,
  'final stability suite must remain wired',
);
assert.equal(
  packageJson.scripts['test:release-ux-gameplay-polish'],
  'node tests/release-ux-gameplay-polish.mjs',
  'release UX/gameplay polish suite must remain wired',
);
assert.equal(
  packageJson.scripts['test:runner-runtime-stability'],
  'node tests/runner-runtime-stability-contract.mjs',
  'RunnerScene runtime stability suite must remain wired',
);
...
assert.equal(
  hardening.endsWith('npm run build'),
  true,
  'release hardening must finish with the production build',
);
assert.equal(
  packageJson.engines?.node,
  '24.x',
  'release Node runtime must stay pinned to Vercel runtime',
);
assert.match(config, /export default defineConfig/);
assert.doesNotMatch(
  config,
  /patch(DeathReason|InitialSpawnShield|CheckpointCollectibles|RespawnTransientState|SeasonalProgression|SpecialEventCreditReward)/,
);
assert.doesNotMatch(
  config,
  /relay-(death-reason|initial-spawn-shield|checkpoint-collectibles|respawn-transient-state|cargo-state-import|runner-zoom-stability)-fix/,
);
...
assert.match(
  runner,
  /const targetZoom\s*=\s*Math\.max\(\s*cinematicTargetZoom\s*,\s*speedZoomTarget\s*\);/,
);
assert.doesNotMatch(
  runner,
  /this\.cameras\.main\.zoom\s*=/,
  'RunnerScene must not directly assign camera zoom',
);
...
assert.match(
  cargo,
  /^import \{ packages \} from '\.\/src\/packages\.js';\r?\nimport \{ loadState, saveState \} from '\.\/src\/state\.js';/,
);
assert.doesNotMatch(
  cargo,
  /import\('\.\/src\/state\.js'\)/,
  'cargo runtime must not lazy-load state through a build patch',
);
assert.match(cargo, /const state = loadState\(\);/);
assert.match(cargo, /saveState\(state\);/);

await assert.rejects(
  access(fileURLToPath(new URL('../vite.config.js', gameRoot))),
  /ENOENT/,
  'legacy Vite config must not return',
);
