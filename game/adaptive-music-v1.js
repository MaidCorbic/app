 ...
 * - Context is created/resumed only after unlock().
 * - Compatible with the existing relay audio autoplay guard.
 */
(() => {
  'use strict';
   ...
   * This prevents Chrome/Safari autoplay warnings during page boot.
   */
  ...
  /*
   * Unlock audio after a trusted user gesture.
   *
   * Uses the existing global relay audio guard when available,
   * then resumes this module's AudioContext.
   */
  ...
  function master(value, fade = 0.25) {
    ...
    try {
      ...
      s.master.gain.setValueAtTime(
        Math.max(0.0001, s.master.gain.value),
        t
      );

      s.master.gain.linearRampToValueAtTime(
        clamp(value, 0.0001, 0.8),
        t + fade
      );
    } catch {}
  }
  ...
  function schedule() {
    ...
    const beat = 60 / bpm;
    const until = s.ctx.currentTime + 0.35;
  ...
  function setIntensity(
    ...
    if (s.filter && s.ctx) {
      try {
        ...
        s.filter.frequency.cancelScheduledValues(
          s.ctx.currentTime
        );

        s.filter.frequency.linearRampToValueAtTime(
          cutoff,
          s.ctx.currentTime + 0.28
        );
      } catch {}
    }
  }
  ...
  if (
    ...
    !RunnerScene.prototype
      .__relayAdaptiveMusicV3Wrapped
  ) {
    RunnerScene.prototype.create =
      function adaptiveArcadeMusicCreate(
        ...args
      ) {
        const result =
          original.apply(
    ...
    RunnerScene.prototype
      .__relayAdaptiveMusicV3Wrapped = true;
  }
  ...
  /*
   * Trusted user gesture handlers.
   * These are passive because they do not call preventDefault().
   */
  const gesture = () => {
    unlock();
  };
