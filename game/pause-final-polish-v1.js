...
(() => {
  ...
  function renderOptions() {
    ...
    panel.querySelectorAll('[data-pause-option]').forEach(btn => btn.addEventListener('click', () => { const key = btn.dataset.pauseOption; const current = state(); const value = key === 'muted' ? !current.muted : !current[key]; save({ [key]: value }); if (key === 'aiVoice' && !value) window.speechSynthesis?.cancel?.(); window.dispatchEvent(new CustomEvent('relay-settings-change', { detail: { key, value } })); renderOptions(); }));
    panel.querySelectorAll('[data-pause-range]').forEach(input => input.addEventListener('input', () => { const value = Number(input.value); save({ [input.dataset.pauseRange]: value }); const label = panel.querySelector(`[data-pause-volume="${input.dataset.pauseRange}"]`); if (label) label.textContent = `${Math.round(value * 100)}%`; }));
