...
(() => {
  ...
  style.textContent = `
    ...
    #pauseMenu #panelContent.pause-progress-compact>*>*{box-sizing:border-box}
    #pauseMenu #panelContent.pause-progress-compact>.progress-card,
    #pauseMenu #panelContent.pause-progress-compact>.progress-panel,
    #pauseMenu #panelContent.pause-progress-compact>.progress-summary,
    #pauseMenu #panelContent.pause-progress-compact>.progress-grid{width:min(92%,620px)!important;max-width:620px!important;margin-inline:auto!important}
    #pauseMenu #panelContent.pause-progress-compact .progress-card,
    #pauseMenu #panelContent.pause-progress-compact .progress-panel{padding:12px!important}
    #pauseMenu.pause-options-open #panelContent{overflow-y:auto!important}
    .pause-options-v1{display:grid;gap:8px;width:100%;max-width:100%;box-sizing:border-box;text-align:left}
    ...
    .pause-options-v1 .toggle{width:92px;min-width:92px;height:34px;border:1px solid rgba(210,226,240,.18);border-radius:7px;background:#07111ff2;color:#e9f2f8;font:800 8px 'DM Mono',monospace;letter-spacing:.6px;cursor:pointer;touch-action:manipulation}
    .pause-options-v1 .toggle.is-on{border-color:rgba(104,231,190,.55);color:#68e7be}.pause-options-v1 .toggle:hover,.pause-options-v1 .toggle:focus-visible{border-color:#ffd06e;color:#ffd06e;outline:none}
  ...
  const button = (label, key, enabled, detail) => `<div class="row"><div class="copy"><b>${label}</b><small>${detail}</small></div><button class="toggle ${enabled ? 'is-on' : ''}" type="button" data-pause-option="${key}" aria-pressed="${enabled}">${enabled ? 'ON' : 'OFF'}</button></div>`;
  ...
  function renderOptions() {
    ...
    panel.innerHTML = `<div class="pause-options-v1"><div class="section">GAMEPLAY / GUIDANCE</div>${button('TUTORIAL','tutorialEnabled',s.tutorialEnabled !== false,'Mission guidance and contextual lessons')}${button('GAME AUDIO','muted',!s.muted,'Master game audio')}${button('SCREEN SHAKE','screenShake',!!s.screenShake,'Camera impact feedback')}${button('REDUCED MOTION','reducedMotion',!!s.reducedMotion,'Reduce movement effects')}${button('ATMOSPHERIC RAIN','rain',!!s.rain,'City weather ambience')}<div class="section">VOICE & AUDIO</div>${button('AI VOICE','aiVoice',s.aiVoice !== false,'NIA / MARA spoken game guidance')}<label class="row"><span class="copy"><b>MUSIC</b><small><span data-pause-volume="musicVolume">${Math.round((s.musicVolume ?? .55) * 100)}%</span> VOLUME</small></span><input data-pause-range="musicVolume" type="range" min="0" max="1" step=".05" value="${s.musicVolume ?? .55}"></label><label class="row"><span class="copy"><b>SFX</b><small><span data-pause-volume="sfxVolume">${Math.round((s.sfxVolume ?? .7) * 100)}%</span> VOLUME</small></span><input data-pause-range="sfxVolume" type="range" min="0" max="1" step=".05" value="${s.sfxVolume ?? .7}"></label><div class="section">LANGUAGE</div><div class="row language"><div class="copy"><b>GAME LANGUAGE</b><small>Choose your interface language</small></div><button class="toggle" type="button" data-pause-language>🌐 ${lang[1]}</button><div class="language-menu hidden" data-pause-language-menu>${LANGUAGES.map(([code,name]) => `<button type="button" data-pause-language-code="${code}" class="${code === lang[0] ? 'active' : ''}">${name}</button>`).join('')}</div></div><div class="section">DISPLAY</div><div class="actions"><button type="button" data-pause-fullscreen>FULLSCREEN</button><button type="button" data-pause-reset>RESET OPTIONS</button></div></div>`;

    panel.querySelectorAll('[data-pause-option]').forEach(btn => btn.addEventListener('click', () => {
    ...
    panel.querySelectorAll('[data-pause-range]').forEach(input => input.addEventListener('input', () => {
      const value = Number(input.value); save({ [input.dataset.pauseRange]: value });
      panel.querySelector(`[data-pause-volume="${input.dataset.pauseRange}"]`).textContent = `${Math.round(value * 100)}%`;
    }));
    const langButton = panel.querySelector('[data-pause-language]');
    const langMenu = panel.querySelector('[data-pause-language-menu]');
    langButton?.addEventListener('click', event => { event.stopPropagation(); langMenu?.classList.toggle('hidden'); });
    ...
    panel.querySelector('[data-pause-reset]')?.addEventListener('click', () => { save({ muted:false, musicVolume:.55, sfxVolume:.7, screenShake:true, reducedMotion:false, rain:true, aiVoice:true, tutorialEnabled:true }); setLanguage('en'); window.dispatchEvent(new CustomEvent('relay-settings-change', { detail:{ reset:true } })); renderOptions(); });
  }

  function closePause() {
    const close = document.querySelector('#pauseMenu [data-close]');
  ...
  function init() {
    ...
    document.addEventListener('click', event => {
      if (event.target.closest?.('#pauseMenu [data-tab="progress"]')) {
        window.setTimeout(() => document.getElementById('panelContent')?.classList.add('pause-progress-compact'), 0);
      }
    }, true);
  }
