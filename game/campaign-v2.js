...
if (pause && panel) {
  if (!document.querySelector('link[data-campaign-v2]')) {
    const style = document.createElement('link');
    style.rel = 'stylesheet';
    style.href = './campaign-v2.css';
    style.dataset.campaignV2 = 'true';
    document.head.appendChild(style);
  }

  const nav = pause.querySelector('.menu-grid aside nav');
  let campaignTab = pause.querySelector('[data-tab="campaign"]');
  if (!campaignTab && nav) {
    campaignTab = document.createElement('button');
    campaignTab.className = 'tab';
    campaignTab.type = 'button';
    campaignTab.dataset.tab = 'campaign';
    campaignTab.textContent = 'CAMPAIGN';
    const missionsTab = nav.querySelector('[data-tab="missions"]');
    nav.insertBefore(campaignTab, missionsTab || nav.firstElementChild);
  }
