...
*/
(() => {
  const state = { root: null, hideTimer: 0, lastKey: '', lastAt: 0, bound: false, observed: new WeakSet() };
  ...
  function ensure() {
    ...
    hud.insertBefore(root, progress.nextSibling); state.root = root; return root;
  }

  function classify(message, source) {
    const value = String(message || '').toUpperCase(); if (!value) return null;
    if (source === 'toast') {
  ...
  function show(event) {
    ...
    state.lastKey = key; state.lastAt = now; root.dataset.type = event[3] || 'event';
    root.querySelector('#gameplayEventType').textContent = event[0]; root.querySelector('#gameplayEventTitle').textContent = event[1]; root.querySelector('#gameplayEventDetail').textContent = event[2];
    ...
    clearTimeout(state.hideTimer); state.hideTimer = setTimeout(() => root.classList.remove('is-visible'), event[3] === 'danger' ? 1500 : 1050);
  }
  ...
  function observeNode(node, source) {
    ...
    const observer = new MutationObserver(() => { const next = node.textContent.trim(); if (!next || next === previous) return; previous = next; const event = classify(next, source); if (event) show(event); });
  ...
  function bind() {
    if (state.bound) return; state.bound = true; ensure(); bindAvailable();
    const domObserver = new MutationObserver(bindAvailable);
