  ...
  return {
    ...
    transform(code, id) {
      ...
      const hasHardLandingTdzSignature =
        code.includes('const hardLanding =\n  this.landingTimer > 0 &&') &&
        code.includes("this.game.events.emit(\n  'feedback',\n  hardLanding");
      ...
      if (hasHardLandingTdzSignature || /const hardLanding =\s*this\.landingTimer\s*>\s*0\s*&&\s*this\.fallSpeed\s*>\s*260;/.test(transformed)) {
        const hardLandingDecl = /const hardLanding =\s*this\.landingTimer\s*>\s*0\s*&&\s*this\.fallSpeed\s*>\s*260;/;
        transformed = transformed.replace(hardLandingDecl, '');

        transformed = transformed.replace(
          /(this\.landingTimer\s*=\s*Math\.max\([\s\S]*?\);)(\s*if\s*\(\s*this\.dashTimer\s*>\s*0\s*\))/,
          '$1\n\nconst hardLanding =\n  this.landingTimer > 0 &&\n  this.fallSpeed > 260;\n$2'
        );

        if (!transformed.includes("const hardLanding =\n  this.landingTimer > 0 &&\n  this.fallSpeed > 260;")) {
          throw new Error(`relay-runner-zoom-stability-fix: failed to move hardLanding declaration in ${id}`);
        }
      }
