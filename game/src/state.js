...
function challengeProgress(state, mission, signals, cleanRun, contractComplete, bossDefeated) { const daily = state.daily?.date === today() ? state.daily : { date: today(), progress: {}, claimed: [] }; const monthly = challengeState(state.monthly, monthKey()); const weekly = challengeState(state.weekly, weekKey()); const seasonal = challengeState(state.seasonal, seasonKey()); return { daily: { ...daily, progress: { ...daily.progress, signals: (daily.progress.signals || 0) + signals, contracts: (daily.progress.contracts || 0) + (contractComplete ? 1 : 0), clean: (daily.progress.clean || 0) + (cleanRun ? 1 : 0), dockTime: (daily.progress.dockTime || 0) + (mission.id === 'dead-drop' && signals >= 0 ? 1 : 0) } }, weekly: { ...weekly, progress: { ...weekly.progress, runs: (weekly.progress.runs || 0) + 1, signals: (weekly.progress.signals || 0) + signals, clean: (weekly.progress.clean || 0) + (cleanRun ? 1 : 0) } }, monthly: { ...monthly, progress: { ...monthly.progress, runs: (monthly.progress.runs || 0) + 1, signals: (monthly.progress.signals || 0) + signals, bosses: (monthly.progress.bosses || 0) + (bossDefeated ? 1 : 0) } }, seasonal: { ...seasonal, progress: { ...seasonal.progress, routes: state.completed.includes(mission.id) ? state.completed.length : state.completed.length + 1, mastery: 0, bosses: (seasonal.progress.bosses || 0) + (bossDefeated ? 1 : 0) } } }; }
export function earnedAchievementIds(state) { const masteryCount = Object.values(state.mastery || {}).reduce((total, badges) => total + badges.length, 0); const bossWins = Object.keys(state.missionStats || {}).filter(id => state.achievements?.includes(`boss-${id}`)).length; return [state.achievements?.includes('first-hostile-down') && 'first-hostile-down', state.completed?.length >= 7 && 'route-runner', state.signals >= 250 && 'signal-hunter', state.totalRuns >= 25 && 'night-shift', bossWins >= 5 && 'boss-breaker', masteryCount >= 12 && 'mastery-ace', state.level >= MAX_LEVEL && 'relay-legend'].filter(Boolean); }
export function claimChallenge(state, scope, id) { const definitions = { daily: dailyChallenges, weekly: weeklyChallenges, monthly: monthlyChallenges, seasonal: seasonalChallenges }[scope]; const progress = state[scope]; const challenge = definitions?.find(entry => entry.id === id); if (!challenge || !progress || progress.claimed?.includes(id) || (progress.progress?.[id] || 0) < challenge.target) return state; const xp = state.xp + challenge.xp; state.xp = xp; state.credits = state.credits + challenge.credits; state.level = levelForXp(xp); state[scope] = { ...progress, claimed: [...progress.claimed, id] }; state.achievements = [...new Set([...(state.achievements || []), ...earnedAchievementIds(state)])]; saveState(state); return state; }
...
export function completeMission(state, mission, signals, elapsedMs = 0, runStats = {}) {
  ...
  if (Number(runStats.packageCondition ?? 100) >= 100 && !missionMastery.includes('PERFECT PACKAGE')) missionMastery.push('PERFECT PACKAGE');
  ...
  const packageCondition = Math.max(0, Math.min(100, Number(runStats.packageCondition ?? 100)));
  const packageBonus = Math.max(0, Math.round(packageCondition / 10));
  ...
  const missionStat = { ...previousMission, completed: true, bestScore: Math.max(previousMission.bestScore, score), bestTime: !previousMission.bestTime || elapsedMs < previousMission.bestTime ? elapsedMs : previousMission.bestTime, bestRating: Math.max(previousMission.bestRating || 0, rating), bestSecrets: Math.max(previousMission.bestSecrets || 0, runStats.secrets || 0), bestSignals: Math.max(previousMission.bestSignals || 0, signals), bestPackageCondition: Math.max(previousMission.bestPackageCondition || 0, packageCondition), mastery: missionMastery };
  ...
  const next = { ...state, xp: nextXp, credits: state.credits + credits + campaignCredits + rivalCredits, level: levelForXp(nextXp), missionStats: { ...state.missionStats, [mission.id]: missionStat }, completed: newCompleted, unlockedMissions: missions.filter((_, index) => !missions[index].unlockRequirement || newCompleted.includes(missions[index].unlockRequirement)).map(route => route.id), campaign: { claimedChapters: [...campaignClaimed, ...newCampaignRewards.map(chapter => chapter.id)] }, ...challengeUpdates, discoveredSecrets: state.discoveredSecrets + Math.max(0, (runStats.secrets || 0) - (previousMission.bestSecrets || 0)), districtProgress: district ? { ...state.districtProgress, [district.id]: { missions: Math.max(previousDistrict.missions, previousMission.completed ? previousDistrict.missions : previousDistrict.missions + 1), signals: Math.max(previousDistrict.signals, signals), secrets: Math.max(previousDistrict.secrets, runStats.secrets || 0), bestScore: Math.max(previousDistrict.bestScore, score) } } : state.districtProgress, rivalProgress: rivalMissionIds.has(mission.id) ? { ...state.rivalProgress, encounters: [...new Set([...(state.rivalProgress?.encounters || []), mission.id])], victories: newRivalVictory ? [...previousRivalVictories, mission.id] : previousRivalVictories, wins: (state.rivalProgress?.wins || 0) + (newRivalVictory ? 1 : 0) } : state.rivalProgress, streak, longestStreak: Math.max(state.longestStreak, streak), lastRunDate: firstRunToday ? runDate : state.lastRunDate, lastStreakBonus: streakBonus, lastSignalBonus: signalBonus, totalRuns: state.totalRuns + 1, bestRun: Math.max(state.bestRun, score), mastery: { ...state.mastery, [mission.id]: missionMastery }, lastXpBreakdown: { completion: mission.reward, signals: signalBonus, secrets: secretBonus, optional: optionalBonus, streak: streakBonus, package: packageBonus, packageCondition, modifier: modifierXp, daily: 0, contract: contractXp, campaign: campaignXp, campaignChapters: newCampaignRewards.map(chapter => chapter.id), rival: rivalXp, credits: credits + campaignCredits + rivalCredits, total: totalXp, objectives: completedObjectives.map(objective => objective.label) }, tutorialSeen: true, achievements: [...new Set([...(state.achievements || []), `route-${mission.id}`, cleanRun && `clean-${mission.id}`, signals === mission.signals.length && `signals-${mission.id}`, runStats.enemyDefeats > 0 && 'first-hostile-down', runStats.bossDefeated && `boss-${mission.id}`].filter(Boolean))], abilities: newAbilities.length ? [...state.abilities, ...newAbilities] : state.abilities, lastAbilityUnlock: ['dash', 'doubleJump', 'wallJump'].includes(newAbilities[0]) ? newAbilities[0] : null };
