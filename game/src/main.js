...
/* ============================================================
   INITIAL UI HARDENING
   ============================================================ */

document
  .querySelector('.input-guide')
  ?.replaceChildren(
    ...(() => {
      ...
      parts.forEach((part, index) => {
        ...
        if (part[1]) {
          ...
          second.textContent = part[1];
          fragment.append(second);
        }

        const label = document.createTextNode(` ${part[2]} `);
...
const pauseClose = document.querySelector(
  '#pauseMenu [data-close]'
);

if (pauseClose) {
  pauseClose.innerHTML =
    '<span>ESC</span> CLOSE <b>×</b>';
...
if (gameRoot) {
  if (!$('#worldMap')) {
    gameRoot.insertAdjacentHTML(
      'beforeend',
      `
        <section
          id="worldMap"
          class="world-map hidden"
          aria-label="City district map"
        >
          <header>
            <p class="eyebrow">CITY RELAY NETWORK</p>
            <h2>CHOOSE A <em>DISTRICT.</em></h2>
            ...
            <nav class="board-tabs">
              <button type="button" data-board="districts">
                DISTRICTS
              </button>

              <button type="button" data-board="missions">
                MAIN MISSIONS
              </button>

              <button type="button" data-board="contracts">
                CONTRACTS
              </button>

              <button type="button" data-board="challenges">
                CHALLENGES
              </button>

              <button type="button" data-board="events">
                SPECIAL EVENTS
              </button>

              <button type="button" data-board="npcs">
                CONTACTS
              </button>
            </nav>
          </header>
      ...
      `
        ...
        >
          <div class="preflight-card">
            ...
            <p class="eyebrow">PRE-FLIGHT LOADOUT</p>

            <h2 id="preflightTitle"></h2>

            <p id="preflightBrief"></p>

            <div id="preflightOptions"></div>
...
/*
 * RunnerScene must NOT auto-start.
 *
 * The scene stays outside the initial Phaser scene array and is
 * registered manually with autoStart=false.
 */
  ...
  event => {
    ...
    if (runner?.infoCard) {
      ...
      try {
        ...
        console.warn(
          '[RelayRunner] Failed to dismiss intel card:',
          error
        );
      }
    }
...
/*
 * Mobile input ownership remains in the dedicated mobile input
 * system. No duplicate action dispatcher is created here.
 */
...
function getAudioContext() {
  ...
  try {
    ...
    console.warn(
      '[RelayRunner] AudioContext unavailable:',
      error
    );
...
function playTone(
  ...
  try {
    ...
    console.warn(
      '[RelayRunner] Tone playback failed:',
      error
    );
  }
}
...
function startAudioBed(
  ...
  try {
    ...
    console.warn(
      '[RelayRunner] Mission audio failed:',
      error
    );
  }
}
...
function speakCharacterResponse(
  text
) {
  const intro =
    $('#intro');
...
game.events.on(
  ...
  () => {
    ...
    if (!hudRun || !hudActions) {
      return;
    }

    if (!$('#energyBar')) {
      hudRun.insertAdjacentHTML(
        'beforeend',
        `
          <div class="hud-vital energy">
            <span>
              <small>ENERGY</small>
    ...
    if (!$('#healthBar')) {
      hudActions.insertAdjacentHTML(
        'afterbegin',
        `
          <div class="hud-vital health">
            <span>
              <small>HEALTH</small>
              <b id="healthValue">3 / 3</b>
            </span>
            <div>
              <i
                id="healthBar"
    ...
    if (!$('#ammoBar')) {
      hudActions.insertAdjacentHTML(
        'afterbegin',
        `
          <div class="hud-vital plasma">
            <span>
              <small>PLASMA</small>
              <b id="ammoValue">READY</b>
            </span>
            <div>
              <i
                id="ammoBar"
    ...
    if (!$('#comboValue')) {
      hudActions.insertAdjacentHTML(
        'afterbegin',
        `
          <div class="hud-vital combo">
            <span>
              <small>FLOW</small>
              <b id="comboValue">READY</b>
            </span>
            <div>
              <i
                id="comboBar"
    ...
    if (
      ...
      hudRoute.insertAdjacentHTML(
        'beforeend',
        `
          <small id="detectionStatus">
            STEALTH · CLEAR
          </small>
        `
      );
    ...
    if (
      ...
      intel.textContent =
        `ROUTE · 0/${scene.mission.checkpoints.length} CHECKPOINTS`;
      ...
      if (!packageMeter) {
        document
          ...
          ?.insertAdjacentHTML(
            'afterbegin',
            `
              <div class="hud-vital package">
                <span>
                  <small>PACKAGE</small>
                  <b id="packageValue">100%</b>
                </span>
                <div>
                  <i
                    id="packageCondition"
...
function openWorldMap() {
  ...
  try {
    districts.forEach(
      district => {
        ...
        card.innerHTML = `
          <span>
            ${
              unlocked
                ...
                : `LOCKED · COMPLETE ${district.unlockMission}`
            }
          </span>
          ...
          <small>
            HAZARDS · ${district.hazards}
          </small>

          <small>
            ENEMIES · ${district.enemies}
          </small>
          ...
          <footer>
            <b>
              ${progress.completed}/${district.missions.length} MISSIONS
            </b>

            <b>
              ${progress.signals} SIGNALS ·
              ${progress.secrets} SECRETS
            </b>

            <b>
              ${progress.contractsDone} CONTRACTS
            </b>
          </footer>
        `;
    ...
    console.error(
      '[RelayRunner] World Map render failed:',
      error
    );

    grid.innerHTML = `
      <article class="job-card">
        <span>CITY RELAY NETWORK</span>
        <h3>MAP RECOVERY</h3>
        <p>
          The map data could not load.
          Restart the briefing to retry.
        </p>
      </article>
    `;
  }
...
function openPreflight(job) {
  ...
  let optionalAbilities =
    0;

  options.innerHTML = `
    <small>
      REQUIRED ABILITIES STAY EQUIPPED ·
    ...
    cards =
      contracts.map(
        contract => ({
          ...
          body:
            `District job · ${contract.xp} XP`,

          reward:
            `${contract.credits} CREDITS`,
    ...
    cards =
      dailyChallenges.map(
        challenge => ({
          ...
          reward:
            `${challenge.xp} XP · ${challenge.credits} CREDITS`
        })
      );
  } else {
    cards =
                ...
                undefined
                  ...
                  : card.modifier
                    ? `
                      <button
                        type="button"
                        data-modifier-job="${card.modifier.id}"
                      >
                        ACTIVATE →
                      </button>
                    `
...
function renderSpecialEvent() {
  ...
  grid.innerHTML = `
    <article class="job-card special-event">
      <span>
        ROTATING EVENT · ${event.availability}
      </span>
        ...
        npc => {
          ...
          return `
            <article class="job-card">
              ...
              <span>
                ${npc.type} · ${npc.district.toUpperCase()}
              </span>
...
function launch(
  ...
  try {
    ...
    console.error(
      '[RelayRunner] Scene launch failed:',
      error
    );
...
function fail(
  ...
  if ($('#failLine')) {
    $('#failLine').textContent =
      message ||
      'The live barrier cut the delivery short.';
  }
          ...
          body: `
            <div class="title-settings">
              ...
              <label>
                ...
                <button
                  type="button"
                  data-title-setting="reducedMotion"
                >
                  ${
                    state.reducedMotion
                      ? 'ON'
              ...
              <p>
                <b>KEYBOARD</b>
                A / D run, SPACE jump, E fire, Q blade.
                <br>
                <b>TOUCH</b>
                drag left control to move, then tap FIRE,
                JUMP, BLADE, or DASH.
              </p>
  ...
  window.setTimeout(
    () => {
      ...
      try {
        ...
        console.error(
          '[RelayRunner] Home transition failed:',
          error
        );
      }
    },
  ...
  journal.insertAdjacentHTML(
    ...
    `
      <article class="entry">
        ...
        <div>
          <b>
            COURIER SYSTEMS ONLINE
          </b>

          <small>
            CAMPAIGN
            ${campaignCleared}/${campaignChapters.length}
            · RIVAL
            ${state.rivalProgress?.wins || 0}/3
            · MASTERY
            ${masteryCount}
          </small>
          ...
          <small>
            CONTRACTS
            ${contractsComplete}/${contracts.length}
            · SECRETS
            ${state.discoveredSecrets}
            · DISTRICTS
            ${state.unlockedDistricts?.length || 0}/${districts.length}
          </small>
        </div>
...
function appendAchievements() {
  ...
  journal.insertAdjacentHTML(
    'beforeend',
    `
      <article class="entry achievement-entry">
...
function appendEnemyCodex() {
  ...
  journal.insertAdjacentHTML(
    'beforeend',
    `
      <article class="entry codex-entry">

        <span>CODEX</span>

        <div>
          <b>
            ${discovered.length}/${Object.keys(
              enemyIntel
            ).length}
            THREATS DECODED
          </b>
...
function renderPanel(
  ...
  if (tab === 'resume') {
    area.innerHTML = `
      <p class="panel-copy">
        Your boots are still on the roof.
        The relay is still lit.
        The city is waiting for the next step.
      </p>
                ...
                >
                  ...
                  <div>
                    ...
                    <small>
                      ${
                        unlocked
                          ? `${mission.signals.length} SIGNALS · ${mission.objective}`
                          : `UNLOCK: Complete ${String(
                              mission.unlockRequirement ||
                                ''
                            ).replaceAll(
                              '-',
                      ...
                      ${
                        stat?.completed
                          ? `RATING ${'★'.repeat(
                              stat.bestRating ||
                                0
                            )} · BEST ${stat.bestScore} · ${formatTime(
                              stat.bestTime
                            )}`
                          : mission.description
                      }
        ...
        <article class="entry">
          ...
          <div>
            ...
            <small>
              ...
              ${
                rank.next
                  ? `${rank.next.threshold - state.xp} XP to ${rank.next.name}`
                  : 'Maximum courier rank.'
              }
            </small>
    ...
    <div class="journal">
      ...
      <article class="entry">
        ...
        <div>
          ...
          <small>
            Spend on optional, capped upgrades.
          </small>
        </div>
    ...
    `
      <article class="entry">
        ...
        <div>
          ...
          <small>
            MELEE SWORD IS ALWAYS AVAILABLE
            WITH <b>Q</b>.
          </small>
      ...
      ${campaignChapters
        .map(
          (
            ...
            const claimed =
              state.campaign?.claimedChapters?.includes(
                chapter.id
              );
                  ...
                  <small>
                    ${
                      claimed
                        ...
                        : completed
                          ? `CHAPTER REWARD · +${chapter.reward.xp} XP · +${chapter.reward.credits} CREDITS`
                          : `REWARD · +${chapter.reward.xp} XP · +${chapter.reward.credits} CREDITS`
                    }
                  </small>
    ...
    `
      <article class="entry">
        ...
        <div>
          ...
          <b>
            ${rivalOperations.name}
            ·
            ${rivalOperations.title}
          </b>

          <small>
            ${rivalOperations.dossier}
          </small>

          <small>
            OPERATIONS
            ${encounters}/${rivalOperations.missions.length}
            ·
            VICTORIES
...
function renderChallenges() {
  ...
  area.innerHTML = `
    <div class="journal">
      ...
      <article class="entry login-reward">
        ...
        <div>
          ...
          <small>
            ...
            Consecutive days increase Relay Gels.
          </small>
...
game.events.on(
  ...
  (
    ...
    toast(
      lost
        ? `CHECKPOINT RESTORED · ${lost} PICKUP${
            lost === 1
              ? ''
              : 'S'
          } LOST`
...
/*
 * Start RunnerScene in paused briefing state.
 *
 * This preserves the existing architecture where the game scene
 * exists for runtime/UI integration but does not run gameplay
 * until the player actually starts a mission.
 */
launch(
  0,
  true
