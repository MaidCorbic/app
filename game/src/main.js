...
document.querySelector('.input-guide').innerHTML =
  '<kbd>A</kbd><kbd>D</kbd> RUN <i></i><kbd>SPACE</kbd> JUMP <i></i><kbd>E</kbd> FIRE <i></i><kbd>Q</kbd> BLADE <i></i><kbd>SHIFT</kbd> DASH';
...
// Therefore RunnerScene is registered manually with autoStart=false.
const game = new Phaser.Game({
  ...
  // MOBILE FIX:
  // Desktop keeps the original RESIZE behaviour.
...
function playFeedback(kind) {
  ...
  const notes = {
    jump: [250, 0.055, 'triangle'],
    wallJump: [310, 0.07, 'triangle'],
    vault: [285, 0.06, 'triangle'],
    slide: [145, 0.07, 'sawtooth'],
    ledgeGrab: [330, 0.06, 'triangle'],
    dash: [360, 0.07, 'sawtooth'],
    gadget: [610, 0.08, 'sine'],
    empty: [95, 0.11, 'square'],
    warning: [180, 0.11, 'square'],
    chase: [155, 0.12, 'sawtooth'],
    land: [105, 0.045, 'sine'],
...
function speakNarration(text) {
  ...
  try {
    speakNarration.lastAt = Date.now();
...
function applyRuntimeSettings() {
  ...
  if (scene) {
    ...
    scene.motionReduced = state.reducedMotion;
  }
}
...
game.events.on(
  ...
  () => {
     ...
     * Do not restart the old procedural audio bed here.
     */
            ...
            <span>
              ${
                unlocked
                  ...
                  : `UNLOCK: ${district.unlockMission.replaceAll('-', ' ')}`
              }
            </span>

            <h3>${district.name}</h3>

            <p>${district.identity}</p>

            <small>HAZARDS · ${district.hazards}</small>
            <small>ENEMIES · ${district.enemies}</small>

            <div class="district-progress">
              <i style="width:${unlocked ? progress.percent : 0}%"></i>
            </div>
      ...
      ? {
          ...
          body: `
            <div class="title-settings">
              <label>
                GAME AUDIO
                <button data-title-setting="muted">
                  ${state.muted ? 'OFF' : 'ON'}
                </button>
              </label>
              ...
              <label>
                REDUCED MOTION
                <button data-title-setting="reducedMotion">
              ...
              <label>
                ATMOSPHERIC RAIN
                <button data-title-setting="rain">
                  ${state.rain ? 'ON' : 'OFF'}
                </button>
              </label>
      ...
      <article class="entry">
        ...
        <div>
          ...
          <small>
            ...
            · DISTRICTS
            ${state.unlockedDistricts.length}/${districts.length}
          </small>
        </div>
  ...
  journal.insertAdjacentHTML(
    ...
    `
      <article class="entry codex-entry">
        ...
        <div>
          <b>
            ${discovered.length}/${Object.keys(enemyIntel).length}
            THREATS DECODED
          </b>
...
function renderPanel(tab) {
  ...
  if (tab === 'resume') {
    area.innerHTML =
      '<p class="panel-copy">Your boots are still on the roof. The relay is still lit. The city is waiting for the next step.</p><button id="resume" class="primary">RESUME RUN <b>→</b></button>';
  }
                ...
                >
                  ...
                  <div>
                    ...
                    <small>
                      ${
                        stat?.completed
                          ? `RATING ${'★'.repeat(stat.bestRating || 0)} · BEST ${stat.bestScore} · ${formatTime(stat.bestTime)}`
                          : mission.description
                      }
                    </small>
            ...
            <small>
              ${
                level.level === 100
                  ...
                  : `${level.next - state.xp} XP to level ${level.level + 1}.`
              }
            </small>
