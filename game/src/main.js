...
$('start').onclick = () => { startAudioBed(); leaveHome(() => launch(0, false)); }; $('continue').onclick = () => { startAudioBed(); leaveHome(() => launch(nextMissionIndex())); }; $('pause').onclick = () => openMenu(); $('returnTitle').onclick = () => { stopAudioBed(); $('pauseMenu').classList.add('hidden'); launch(0, true); }; $('again').onclick = () => { startAudioBed(); $('finish').classList.add('hidden'); launch(missionIndex); }; $('nextMission').onclick = () => { startAudioBed(); $('finish').classList.add('hidden'); launch(missionIndex + 1); }; $('finishTitle').onclick = () => { stopAudioBed(); $('finish').classList.add('hidden'); launch(0, true); }; $('retry').onclick = () => { startAudioBed(); $('gameOver').classList.add('hidden'); launch(missionIndex); }; $('failTitle').onclick = () => { stopAudioBed(); $('gameOver').classList.add('hidden'); launch(0, true); }; document.querySelectorAll('.tab').forEach(button => button.onclick = () => renderPanel(button.dataset.tab)); document.querySelectorAll('[data-close]').forEach(button => button.onclick = closeMenu); document.addEventListener('keydown', event => { if (event.repeat) return; if ((event.key === 'Enter' || event.code === 'Space') && !$('gameOver').classList.contains('hidden')) { event.preventDefault(); $('retry').click(); } if (event.key === 'Enter' && !$('intro').classList.contains('hidden')) $('start').click(); if (event.key === 'Escape' && game.scene.isActive('runner') && $('intro').classList.contains('hidden') && $('finish').classList.contains('hidden') && $('gameOver').classList.contains('hidden')) { if ($('pauseMenu').classList.contains('hidden')) openMenu(); else closeMenu(); } });
