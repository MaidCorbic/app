...
function drawZone(scene, x, y) {
  return scene.add.circle(x, y, 58, 0x76e07b, .10).setStrokeStyle(2, 0x8df4ff, .45).setDepth(4);
}

function drawStation(scene, x, y) {
  return scene.add.rectangle(x, y, 34, 70, 0x14283a, 1).setStrokeStyle(2, 0x8df4ff, .85).setDepth(7);
}
...
export function installBiohazardContamination(RunnerScene) {
  ...
  RunnerScene.prototype.create = function (...args) {
    ...
    // Encounter is placed ahead of the player without creating a new HUD or route system.
    ...
    const clean = () => {
      state.exposure = 0;
      state.contaminatedAt = 0;
      setStage(STAGES.CLEAN);
      this.events?.emit?.('biohazard:decontaminated');
    };
    ...
    const onUpdate = (_time, delta = 16) => {
      ...
      if (inside && state.stage < STAGES.CONTAMINATED) {
        ...
        if (state.exposure >= EXPOSURE_MS) {
          state.contaminatedAt = now;
          setStage(STAGES.CONTAMINATED);
        } else if (state.stage === STAGES.CLEAN) {
          setStage(STAGES.EXPOSED);
        }
      }

      if (state.stage === STAGES.CONTAMINATED && now - state.contaminatedAt >= CRITICAL_MS) setStage(STAGES.CRITICAL);
      if (state.stage >= STAGES.CONTAMINATED) {
        state.pulse += delta;
        player.setAlpha(.82 + Math.sin(state.pulse * .012) * .08);
      } else if (state.stage === STAGES.CLEAN) {
        player.setAlpha(1);
      }
