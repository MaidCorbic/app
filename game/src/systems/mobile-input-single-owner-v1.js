...
const installRunnerMovementBridge = scene => {
  ...
  const apply = () => {
    ...
    const maxSpeed = Number(scene.__relayMobileRunSpeed) > 0 ? Number(scene.__relayMobileRunSpeed) : 600;
    const target = axis * maxSpeed;
    const current = Number(body.velocity?.x) || 0;
    body.setVelocityX?.(clamp(current + (target - current) * 0.72, -maxSpeed, maxSpeed));
  };
  scene.events.on('postupdate', apply);
  scene.events.once('shutdown', () => { scene.events.off('postupdate', apply); scene.__relayMobileMovementBridgeV11 = false; scene.__relayMobileAxis = 0; });
};
const attachSceneWhenReady = () => { const scene = window.__relayRunnerScene; if (!scene) return false; detachLegacyRunnerInput(scene); installRunnerMovementBridge(scene); return true; };
window.addEventListener('relay:runner-scene-ready', event => { const scene = event?.detail?.scene || window.__relayRunnerScene; if (scene) { detachLegacyRunnerInput(scene); installRunnerMovementBridge(scene); } });
const normalizeActionButtons = root => { const seen = new Set(); root.querySelectorAll('[data-mobile-action]').forEach(node => { const action = node.dataset.mobileAction; if (!ACTION_KEYS[action] || seen.has(action)) node.remove(); else seen.add(action); }); };
const install = () => {
  ...
  const waitForScene = () => { if (!attachSceneWhenReady()) requestAnimationFrame(waitForScene); };
  waitForScene();
  ...
  const releaseAction = (button, pointerId) => {
    ...
    pointers.delete(pointerId); pointerActions.delete(pointerId);
    if (pointers.size === 0) {
  ...
  actionButtons.forEach(button => {
    ...
    button.addEventListener('pointerdown', event => {
      ...
      pointers.add(event.pointerId); pointerActions.set(event.pointerId, action);
  ...
  const releaseAll = () => { for (const [action, pointers] of actionPointers) if (pointers.size && ACTION_KEYS[action]) emit(ACTION_KEYS[action], 'keyup'); actionPointers.clear(); pointerActions.clear(); actionButtons.forEach(button => { button.classList.remove('is-active'); button.setAttribute('aria-pressed', 'false'); }); };
  ...
  const setPhaserDirection = next => {
    ...
    const down = next === 'left'; const right = next === 'right';
    const keys = scene.keys || {}; const cursors = scene.cursors || {};
    if (keys.A) keys.A.isDown = down; if (keys.D) keys.D.isDown = right;
    if (cursors.left) cursors.left.isDown = down; if (cursors.right) cursors.right.isDown = right;
  ...
  const reset = () => {
    ...
    if (scene) { scene.__relayMobileAxis = 0; const body = scene.player?.body; if (body?.enable !== false && body?.moves !== false) body.setVelocityX?.(0); }
    pointerId = null; joystick.classList.remove('is-active'); thumb.style.transform = 'translate(0,0)';
  };
  const move = (clientX, clientY) => {
    const rect = joystick.getBoundingClientRect();
    const dx = clientX - rect.left - rect.width / 2; const dy = clientY - rect.top - rect.height / 2;
    const distance = Math.min(Math.hypot(dx, dy), maxDrag); const angle = Math.atan2(dy, dx);
  ...
  joystick.addEventListener('pointerdown', event => { event.preventDefault(); event.stopPropagation(); if (pointerId !== null) return; pointerId = event.pointerId; joystick.setPointerCapture?.(pointerId); joystick.classList.add('is-active'); move(event.clientX, event.clientY); }, { passive: false });
  joystick.addEventListener('pointermove', event => { if (event.pointerId !== pointerId) return; event.preventDefault(); move(event.clientX, event.clientY); }, { passive: false });
