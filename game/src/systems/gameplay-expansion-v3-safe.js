...
function bindKey(scene, code, fn) {
  const name = `keydown-${code}`;
  scene.input.keyboard?.on(name, fn);
  state(scene).bindings.push([name, fn]);
}
...
function installBodySwap(scene) {
  ...
  const base = { scaleX: player.scaleX, scaleY: player.scaleY, width: player.body.width, height: player.body.height, offsetX: player.body.offset.x, offsetY: player.body.offset.y, mass: player.body.mass };
  ...
  const e = { active: false, base, host, badge };
  st.entities.bodySwap = e;
  const toggle = () => {
    e.active = !e.active;
    if (e.active) {
      ...
      player.setTint(0xffd06e); e.badge.setText('HOST BODY · B'); cue(scene, 'BODY SWAP — HOST BODY', '#ffd06e');
    } else {
      player.setScale(base.scaleX, base.scaleY); player.body.setSize(base.width, base.height, true); player.body.setOffset(base.offsetX, base.offsetY); player.body.setMass(base.mass || 1); player.clearTint(); host.setVisible(false);
      e.badge.setText('BODY SWAP · B'); cue(scene, 'BASE BODY RESTORED', '#8df4ff');
    }
  };
  e.toggle = toggle; remember(scene, button(scene, 108, 94, 'SWAP BODY', toggle, '#ffd06e')); bindKey(scene, 'B', toggle);
}
function updateBodySwap(scene) { const e = scene[NS]?.entities.bodySwap; if (e?.active && scene.player?.active) e.host.setPosition(scene.player.x - 12, scene.player.y).setVisible(true); }
...
function installClonePosition(scene) {
  ...
  const e = { clone, playerPlate, clonePlate, gate, active: false, expiresAt: 0, badge }; st.entities.clonePosition = e;
  const deploy = () => {
    if (e.active) { e.active = false; clone.setVisible(false); cue(scene, 'CLONE RECALLED', '#e0a7ff'); return; }
    e.active = true; e.expiresAt = performance.now() + 7000; clone.setPosition(scene.player.x, scene.player.y).setVisible(true); cue(scene, 'CLONE DEPLOYED', '#e0a7ff');
  };
  e.deploy = deploy; remember(scene, button(scene, x1, 130, 'DEPLOY CLONE', deploy, '#e0a7ff')); bindKey(scene, 'C', deploy);
}
function updateClonePosition(scene) {
  ...
  const onPlayer = Math.abs(scene.player.x - e.playerPlate.x) < 42 && Math.abs(scene.player.y - e.playerPlate.y) < 42;
  const onClone = e.active && Math.abs(e.clone.x - e.clonePlate.x) < 42 && Math.abs(e.clone.y - e.clonePlate.y) < 42;
  const open = onPlayer && onClone;
  e.gate.setAlpha(open ? 0.18 : 0.9); e.gate.setStrokeStyle(2, open ? 0xaee37f : 0x8df4ff, 0.65); e.badge.setText(open ? 'CLONE SYNC · PATH OPEN' : 'CLONE POSITION · C');
}
...
function installMassTransfer(scene) {
  ...
  const anchor = remember(scene, scene.physics.add.sprite(x, y, 'gxv3-anchor').setDepth(12).setImmovable(true)); anchor.body.allowGravity = false; anchor.body.setMass(5.5);
  ...
  const badge = label(scene, 'MASS TRANSFER · X', x, y - 40, '#ff9c91', '9px');
  const e = { active: false, anchor, beam, baseMass, badge }; st.entities.massTransfer = e;
  const toggle = () => {
    e.active = !e.active; scene.player.body.setMass(e.active ? Math.max(0.35, baseMass * 0.35) : baseMass); anchor.body.setMass(e.active ? 5.5 : 1.0); anchor.setTint(e.active ? 0xff826e : 0xffffff); e.badge.setText(e.active ? 'MASS TRANSFERRED · X' : 'MASS TRANSFER · X'); cue(scene, e.active ? 'MASS TRANSFERRED' : 'MASS RETURNED', e.active ? '#ff826e' : '#8df4ff');
  };
  e.toggle = toggle; remember(scene, button(scene, x, y + 58, 'TRANSFER MASS', toggle, '#ff826e')); bindKey(scene, 'X', toggle);
}
function updateMassTransfer(scene) { const e = scene[NS]?.entities.massTransfer; if (!e || !scene.player?.active) return; e.beam.setTo(scene.player.x, scene.player.y, e.anchor.x, e.anchor.y); if (e.active && Phaser.Math.Distance.Between(scene.player.x, scene.player.y, e.anchor.x, e.anchor.y) > 360) e.toggle(); }

function installPhaseSplit(scene) {
  const st = state(scene); if (!st.enabled.phaseSplit || st.entities.phaseSplit || !scene.player) return;
  const echo = remember(scene, scene.physics.add.sprite(scene.player.x - 150, scene.player.y, 'gxv3-phase-echo').setDepth(11).setVisible(false).setAlpha(0.55)); echo.body.allowGravity = false; echo.body.immovable = true;
  const badge = label(scene, 'PHASE SPLIT · V', routeX(scene, 0.44), 142, '#8df4ff', '9px').setScrollFactor(0);
  ...
  const e = { active: false, echo, beacon, badge, lastX: scene.player.x }; st.entities.phaseSplit = e;
  const toggle = () => { e.active = !e.active; echo.setVisible(e.active); beacon.setStrokeStyle(2, e.active ? 0xe0a7ff : 0x8df4ff, 0.7); cue(scene, e.active ? 'PHASE SPLIT ACTIVE' : 'PHASE SPLIT OFF', e.active ? '#e0a7ff' : '#8df4ff'); };
  e.toggle = toggle; remember(scene, button(scene, routeX(scene, 0.44), 164, 'SPLIT PHASE', toggle, '#8df4ff')); bindKey(scene, 'V', toggle);
}
function updatePhaseSplit(scene) { const e = scene[NS]?.entities.phaseSplit; if (!e || !scene.player?.active) return; const dx = scene.player.x - e.lastX; if (e.active) { e.echo.x += -dx * 0.8; e.echo.y = scene.player.y + Math.sin(performance.now() / 150) * 5; } e.lastX = scene.player.x; e.badge.setText(e.active ? 'PHASE SPLIT · ECHO LIVE' : 'PHASE SPLIT · V'); }
...
function installObjectFusion(scene) {
  ...
  const a = remember(scene, scene.physics.add.sprite(x - 34, y, 'gxv3-fuse-a').setDepth(11)); const b = remember(scene, scene.physics.add.sprite(x + 34, y, 'gxv3-fuse-b').setDepth(11));
  a.body.allowGravity = b.body.allowGravity = true; a.body.setMass(1); b.body.setMass(1);
  const fused = remember(scene, scene.physics.add.sprite(x, y, 'gxv3-fused').setDepth(11).setVisible(false)); fused.body.allowGravity = true; fused.body.setMass(4);
  const badge = label(scene, 'OBJECT FUSION · N', x, y - 62, '#ffd06e', '9px'); const e = { a, b, fused, active: false, badge }; st.entities.objectFusion = e;
  const fuse = () => { if (e.active) return; e.active = true; a.disableBody(true, true); b.disableBody(true, true); fused.setPosition(x, y).setVisible(true); cue(scene, 'OBJECTS FUSED', '#ffd06e'); };
  e.fuse = fuse; a.setInteractive(); b.setInteractive(); a.on('pointerdown', fuse); b.on('pointerdown', fuse); remember(scene, button(scene, x, y + 60, 'FUSE OBJECTS', fuse, '#ffd06e')); bindKey(scene, 'N', fuse);
}
function updateObjectFusion(scene) { const e = scene[NS]?.entities.objectFusion; if (e) e.badge.setText(e.active ? 'FUSED OBJECT · N' : 'OBJECT FUSION · N'); }
...
function installScaleShift(scene) {
  ...
  const p = scene.player; const base = { scaleX: p.scaleX, scaleY: p.scaleY, width: p.body.width, height: p.body.height };
  const badge = label(scene, 'SCALE SHIFT · Z', routeX(scene, 0.48), 178, '#aee37f', '9px').setScrollFactor(0); const e = { small: false, base, badge }; st.entities.scaleShift = e;
  const toggle = () => { e.small = !e.small; const factor = e.small ? 0.62 : 1; p.setScale(base.scaleX * factor, base.scaleY * factor); p.body.setSize(base.width * factor, base.height * factor, true); e.badge.setText(e.small ? 'SCALE SHIFT · SMALL · Z' : 'SCALE SHIFT · NORMAL · Z'); cue(scene, e.small ? 'SMALL FORM' : 'NORMAL FORM', '#aee37f'); };
  e.toggle = toggle; remember(scene, button(scene, routeX(scene, 0.48), 200, 'SHIFT SCALE', toggle, '#aee37f')); bindKey(scene, 'Z', toggle);
}
...
function installRuleInjection(scene) {
  ...
  const target = remember(scene, scene.physics.add.sprite(x, y, 'gxv3-rule').setDepth(12).setImmovable(true)); target.body.allowGravity = false;
  const modes = [{ id: 'ANCHOR', tint: 0x8df4ff, immovable: true, bounce: 0 }, { id: 'BOUNCE', tint: 0xffd06e, immovable: false, bounce: 1.15 }, { id: 'DRIFT', tint: 0xaee37f, immovable: false, bounce: 0.15 }];
  const badge = label(scene, 'RULE INJECTION · Q', x, y - 48, '#b9f5ff', '9px'); const e = { index: 0, target, modes, badge }; st.entities.ruleInjection = e;
  const apply = () => { e.index = (e.index + 1) % modes.length; const mode = modes[e.index]; target.body.setImmovable(mode.immovable); target.body.setBounce(mode.bounce); target.setTint(mode.tint); if (mode.id === 'DRIFT') target.body.setVelocityX(120); else target.body.setVelocity(0, 0); e.badge.setText(`RULE: ${mode.id} · Q`); cue(scene, `RULE ${mode.id}`, '#b9f5ff'); };
  e.apply = apply; target.setInteractive(); target.on('pointerdown', apply); remember(scene, button(scene, x, y + 66, 'INJECT RULE', apply, '#b9f5ff')); bindKey(scene, 'Q', apply);
}
function updateRuleInjection(scene) { const e = scene[NS]?.entities.ruleInjection; if (e?.modes[e.index].id === 'DRIFT' && Math.abs(e.target.body.velocity.x) < 5) e.target.body.setVelocityX(120); }
...
export function installGameplayExpansionV3Safe(RunnerScene) {
  ...
  RunnerScene.prototype.create = function (...args) {
    ...
    this.events?.once?.('shutdown', () => {
      ...
      this.events?.off?.('update', update); this[NS] = null;
    });
    return result;
  };
}
