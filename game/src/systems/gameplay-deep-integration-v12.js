/* V12 — deep gameplay integration as a state/event layer.
   No developer HUD and no per-frame persistence loop. Reacts to actual gameplay events. */
...
function apply(state,type,detail){const t=(String(type||'')+' '+JSON.stringify(detail||{})).toLowerCase();let changed=false;
  const add=(key,n=1)=>{state[key]=Number(state[key]||0)+n;changed=true;};
  ...
  if(t.includes('route')){state.route=state.route||'safe';changed=true;}
  ...
  if(t.includes('marker')){const p=playerOf(detail?.scene);state.markers=state.markers.concat({x:p?.x||0,y:p?.y||0,t:Date.now()}).slice(-10);changed=true;}
...
export function installGameplayDeepIntegrationV12(RunnerScene){
  ...
  RunnerScene.prototype.create=function gameplayV12Create(...args){
