...
function buildPanel(scene, objective) {
  ...
  const mapLine = scene.add.rectangle(33, 94, 354, 2, colors.cyan, .19)
    .setOrigin(0, .5);

  const mapGlow = scene.add.rectangle(33, 94, 0, 4, colors.cyan, .42)
    .setOrigin(0, .5);

  const mapHot = scene.add.rectangle(33, 93.2, 0, 1.2, colors.cyanBright, .9)
    .setOrigin(0, .5);
  ...
  const routeData = Array.isArray(scene.mission?.checkpoints)
  ...
  for (const cp of routeData.slice(0, 7)) {
    ...
    const innerDot = scene.add.circle(
      x,
      94,
      2,
      colors.violet,
  ...
  const footerLine = scene.add.rectangle(
    24,
    146,
    372,
    1,
  ...
  const footer = scene.add.text(
    24,
    154,
    '● NAVIGATION LOCKED  /  FOLLOW THE NEON TRACE',
    {
      fontFamily: 'monospace',
      fontSize: '7px',
      fontStyle: 'bold',
...
function layout(state, scene, force = false) {
  ...
  const panelW = mobile
    ? Math.min(320, Math.max(250, w - 20))
    : Math.min(420, Math.max(340, w - 48));

  const scale = panelW / 420;
  const actualH = 178 * scale;

  const x = mobile
    ? Math.max(10, w - panelW - 10)
    : Math.max(24, w - panelW - 28);

  const y = mobile
    ? Math.max(74, h - actualH - 98)
...
export function applyMissionObjective(
   ...
   */
  state.c.setVisible(!viewport(scene).phone);
