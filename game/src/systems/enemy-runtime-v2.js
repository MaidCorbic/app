...
function cueAbility(scene, enemy, text, color) {
  ...
  if (label?.active) {
    label.setText(text).setPosition(enemy.x, enemy.y - 52).setAlpha(1);
    scene.tweens.add({ targets: label, y: enemy.y - 70, alpha: 0, duration: 520 });
  }
...
function spawnBoss(scene) {
  ...
  boss.body.setSize(Math.min(boss.width * .72, 72), Math.min(boss.height * .72, 72), true);
  boss.setData('route', { type: profile.type, min: scene.mission.goal.x - 390, max: scene.mission.goal.x - 120 });
  boss.setData('direction', -1);
  boss.setData('health', profile.health);
  boss.setData('boss', true);
  boss.setData('bossName', profile.name);
  boss.setData('bossColor', profile.color);
  boss.setData('attackCooldown', profile.attackCooldown);
  boss.setData('nextShot', scene.elapsedMs + 900);
  boss.setTint(profile.color);
  ...
  boss.setData('indicator', indicator);
  boss.setData('label', label);
  boss.setData('abilityLabel', abilityLabel);
  scene.boss = boss;
}
...
function spawnTutorialEnemies(scene) {
  ...
  const add = (type, x, y, text) => {
    const enemy = scene.enemies.create(x, y, type).setDepth(8).setImmovable(false);
    enemy.body.setAllowGravity(false);
    enemy.body.setSize(Math.min(enemy.width * .72, 34), Math.min(enemy.height * .8, 52), true);
    enemy.setData('route', { type, min: x - 120, max: x + 120 });
    enemy.setData('direction', 1);
    enemy.setData('nextShot', scene.elapsedMs + 1300);
    enemy.setData('tutorial', true);
    ...
    enemy.setData('tutorialLabel', scene.add.text(x, y - 76, text, { fontFamily: 'DM Mono', fontSize: '9px', color: '#b9f5ff', stroke: '#08101c', strokeThickness: 4, align: 'center' }).setOrigin(.5).setDepth(13));
    return enemy;
  };
  add('enemy-runner', scene.mission.spawn.x + 430, scene.mission.spawn.y, 'SCOUT RUNNER · STOMP OR FIRE');
...
function fireEnemyAbility(scene, enemy) {
  const type = typeOf(enemy);
  const profile = PROFILE[type];
  if (!profile || !enemy.active || scene.empTimer > 0 || scene.decoyTimer > 0) return;
...
function applyCharge(scene, enemy) {
  ...
  if (Math.abs(scene.player.x - enemy.x) < 62 && Math.abs(scene.player.y - enemy.y) < 90) {
    enemy.setData('chargeUntil', 0);
    scene.takeSciFiHit(`${typeOf(enemy).toUpperCase()} charge hit the courier.`);
  }
}
...
function updateBoss(scene, delta) {
  ...
  const profile = BOSS_ABILITY[type] || BOSS_ABILITY['sentinel-boss'];
  ...
  const desired = distance > 260 ? direction * 42 : -direction * 24;
  boss.body.setVelocityX(desired);
  if (boss.x <= route.min || boss.x >= route.max) boss.setData('direction', -direction);
  const now = scene.elapsedMs;
  if (distance > 520 || now < (boss.getData('nextShot') || 0) || scene.empTimer > 0 || scene.decoyTimer > 0) return;
  const dx = scene.player.x - boss.x;
  const dy = scene.player.y - boss.y;
  cueAbility(scene, boss, profile.ability, profile.color);
  if (type === 'storm-boss') {
    for (let i = 0; i < profile.count; i++) {
      const angle = -0.9 + i * .45;
      makeProjectile(scene, boss.x, boss.y, 'comet', profile.color, Math.cos(angle) * 360, Math.sin(angle) * 360);
    }
  } else if (type === 'apex-boss') {
    [-.24, -.08, .08, .24].forEach(angle => makeProjectile(scene, boss.x, boss.y, 'comet', profile.color, Math.cos(angle) * 520 * (dx < 0 ? -1 : 1), Math.sin(angle) * 260));
  } else {
    const speed = type === 'dino-boss' ? 430 : 360;
    [-.22, 0, .22].forEach(angle => makeProjectile(scene, boss.x, boss.y, 'comet', profile.color, dx * .45 + Math.cos(angle) * speed, dy * .22 + Math.sin(angle) * speed * .45));
  }
...
function decorateHazards(scene) {
  if (scene.__enemyHazardsDecorated || !scene.barriers) return;
  scene.__enemyHazardsDecorated = true;
  const accent = scene.mission.blackout ? 0x8df4ff : 0xff826e;
  scene.hazardDecor = [];
  scene.barriers.getChildren().forEach((barrier, index) => {
    const color = index % 3 === 0 ? accent : index % 3 === 1 ? 0xffd06e : 0xb993ff;
    const frame = scene.add.rectangle(barrier.x, barrier.y, 54, 70, 0x07101f, .12).setStrokeStyle(2, color, .75).setDepth(9);
    const core = scene.add.circle(barrier.x, barrier.y, 5, color, .7).setDepth(10);
    const label = scene.add.text(barrier.x, barrier.y - 45, index % 3 === 0 ? 'KINETIC' : index % 3 === 1 ? 'LASER' : 'RELAY', { fontFamily: 'DM Mono', fontSize: '7px', color: '#dffcff', stroke: '#08101c', strokeThickness: 3 }).setOrigin(.5).setDepth(10);
    scene.hazardDecor.push({ barrier, frame, core, label });
    if (!scene.motionReduced) scene.tweens.add({ targets: core, scale: { from: .8, to: 1.45 }, alpha: { from: .45, to: .95 }, yoyo: true, repeat: -1, duration: 540 + index * 70 });
  });
}
...
export function installEnemyRuntime(RunnerScene) {
  ...
  const originalCreateSciFiThreats = prototype.createSciFiThreats;
  const originalCreateHazards = prototype.createHazards;
  prototype.createSciFiThreats = function enemyRuntimeCreateSciFiThreats() {
    ...
    this.enemies.getChildren().forEach(enemy => {
      ...
      enemy.body.setAllowGravity(false);
      enemy.setData('baseY', enemy.y);
      enemy.setData('nextShot', this.elapsedMs + 900 + Math.random() * 700);
      enemy.setData('ability', profile.ability);
      enemy.setData('abilityLabel', this.add.text(enemy.x, enemy.y - 54, profile.ability, { fontFamily: 'DM Mono', fontSize: '8px', color: '#ffcf82', stroke: '#08101c', strokeThickness: 3 }).setOrigin(.5).setDepth(13).setAlpha(.4));
    });
    spawnTutorialEnemies(this);
    spawnBoss(this);
    installProjectileProtection(this);
  ...
  prototype.updateEnemies = function enemyRuntimeUpdateEnemies(delta) {
    ...
    this.enemies.getChildren().forEach(enemy => {
      ...
      const alerted = distance < (profile?.range || 180) && Math.abs(this.player.y - enemy.y) < 125;
      indicator?.setFillStyle(alerted ? 0xffd06e : 0xff826e, alerted ? .78 : .28);
      indicator?.setRadius(alerted ? 8 : 5);
    });
    ...
    if (detected) this.alarmTimer = Math.max(this.alarmTimer, this.alarmDuration(1200));
    this.game.events.emit('detection', Math.ceil(this.alarmTimer / 100));
  };
  ...
  prototype.createHazards = function enemyRuntimeCreateHazards() {
    originalCreateHazards.call(this);
    decorateHazards(this);
  };
