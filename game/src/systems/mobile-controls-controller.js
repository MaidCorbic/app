...
function installStyle() {
  ...
  style.textContent = `
    ...
    body.is-touch .relay-mobile-dpad button,
    body.is-touch .relay-mobile-dpad .relay-dpad-center {
    ...
    body.is-touch .relay-mobile-dpad button {
      ...
      background: linear-gradient(145deg,rgba(5,20,35,.96),rgba(4,10,19,.98))!important;
      color: #dffcff!important;
      font: 900 22px/1 ui-monospace,monospace!important;
      box-shadow: 0 0 16px rgba(25,200,245,.12), inset 0 0 12px rgba(141,244,255,.04)!important;
      pointer-events: auto!important;
      touch-action: none!important;
      -webkit-tap-highlight-color: transparent!important;
    }
    body.is-touch .relay-mobile-dpad button.is-active {
      transform: scale(.94)!important;
      border-color: #8df4ff!important;
      background: linear-gradient(145deg,rgba(15,65,86,.98),rgba(5,22,35,.98))!important;
      box-shadow: 0 0 22px rgba(141,244,255,.42), inset 0 0 16px rgba(141,244,255,.12)!important;
    }
    ...
    body.is-touch .mobile-actions [data-mobile-action] {
    ...
    @media(max-width:430px){
      body.is-touch #play .mobile-controls,body.is-touch .mobile-controls{bottom:max(8px,env(safe-area-inset-bottom,0px)+7px)!important;gap:5px!important}
      body.is-touch .relay-mobile-dpad{transform:scale(.84);transform-origin:bottom left}
      body.is-touch .mobile-actions{grid-template-columns:repeat(2,44px)!important;grid-auto-rows:44px!important;gap:5px!important}
      body.is-touch .mobile-actions [data-mobile-action]{width:44px!important;height:44px!important;font-size:8px!important}
    }
...
function ensureDpad(root) {
  ...
  if (!dpad) {
    ...
    dpad.innerHTML = `
      <button type="button" data-mobile-direction="up" aria-label="Jump">▲</button>
      <button type="button" data-mobile-direction="left" aria-label="Move left">◀</button>
      <span class="relay-dpad-center" aria-hidden="true"></span>
      <button type="button" data-mobile-direction="right" aria-label="Move right">▶</button>
      <button type="button" data-mobile-direction="down" aria-label="Move down / slide">▼</button>`;
    const joystick = root.querySelector('[data-mobile-joystick]');
    if (joystick) joystick.replaceWith(dpad);
    else root.insertBefore(dpad, root.firstChild);
  }
  return dpad;
}
...
function ensureActions(root) {
  ...
  [['jump','▲'],['dash','DASH'],['fire','FIRE'],['sword','BLADE']].forEach(([action,label]) => {
...
function bindControls(root) {
  ...
  const pressDirection = (button, event) => {
    event.preventDefault();
    event.stopImmediatePropagation();
    if (document.body.classList.contains('relay-cinematic-active')) return;
    button.classList.add('is-active');
  ...
  // Capture phase is intentional: it prevents legacy button handlers in main.js
  // and older mobile layers from receiving the same pointer event.
  document.addEventListener('pointerdown', event => {
    ...
    if (!root.contains(actionButton || directionButton)) return;
    if (actionButton) pressAction(actionButton, event);
    else pressDirection(directionButton, event);
  ...
  document.addEventListener('pointerup', event => {
    ...
    const directionButton = event.target?.closest?.('[data-mobile-direction]');
    if (!root.contains(actionButton || directionButton)) return;
    if (actionButton) releaseAction(actionButton, event);
    else releaseDirection(directionButton, event);
  }, { capture: true, passive: false });
...
function findPauseButton() {
  return document.getElementById('pauseBtn')
    ...
    || document.querySelector('[data-action="pause"],[data-pause-button]');
}
