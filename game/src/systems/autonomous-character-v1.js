...
export function installAutonomousCharacter(RunnerScene) {
  ...
  RunnerScene.prototype.create = function (...args) {
    ...
    const state = this.__autonomousCharacter = {
      ...
      marker: null,
      scanLine: null,
      baseRotation: player.rotation || 0,
    };
    ...
    const makeScanLine = () => {
      if (state.scanLine?.active) return state.scanLine;
      const line = this.add.graphics().setDepth(6);
      line.lineStyle(2, 0x8df4ff, .28);
      state.scanLine = line;
      return line;
    };

    const drawScan = (angle) => {
      const line = makeScanLine();
      line.clear();
      line.lineStyle(2, 0x8df4ff, .28);
      line.lineBetween(player.x, player.y - 20, player.x + Math.cos(angle) * 92, player.y - 20 + Math.sin(angle) * 92);
    };

    const beginRoutine = () => {
      if (state.routineUntil || !state.active) return;
      state.routine = 'observe';
      state.routineUntil = performance.now() + ROUTINE_MS;
      state.nextScanAt = performance.now();
      state.scanIndex = 0;
      state.baseRotation = player.rotation || 0;
      makeMarker().setPosition(player.x, player.y - 46).setAlpha(0);
      makeScanLine();
      this.playerCue?.('AUTO OBSERVE', 'SYSTEM');
      this.events?.emit?.('character:idle-routine', { routine: state.routine });
      this.events?.emit?.('character:idle-observe', { phase: 'start' });
    };
    ...
    const resetIdle = () => {
      ...
      state.routine = null;
      state.scanIndex = 0;
      player.rotation = state.baseRotation;
      state.marker?.setAlpha(0);
      state.scanLine?.clear();
    };
    ...
    const onUpdate = (_time, delta = 16) => {
      ...
      const remaining = state.routineUntil - now;
      const elapsed = ROUTINE_MS - remaining;
      const marker = makeMarker();
      marker.setPosition(player.x, player.y - 46);
      marker.setAlpha(Math.min(.65, Math.max(0, elapsed / 700)));
      marker.rotation += delta * .0015;

      if (now >= state.nextScanAt) {
        const scanAngles = [-0.48, 0, 0.48, 0];
        const angle = scanAngles[state.scanIndex % scanAngles.length];
        state.scanIndex += 1;
        state.nextScanAt = now + SCAN_INTERVAL_MS;
        player.rotation = state.baseRotation + angle * .08;
        drawScan(angle);
        this.events?.emit?.('character:idle-observe', {
          phase: 'scan',
          direction: angle < 0 ? 'left' : angle > 0 ? 'right' : 'center',
        });
      }

      if (remaining <= 0) {
        this.events?.emit?.('character:idle-observe', { phase: 'complete' });
        resetIdle();
      }
    };
