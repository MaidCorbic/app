...
(() => {
  ...
  const install = () => {
    ...
    const sync = () => gate.classList.toggle('is-visible', isPortrait());
    ...
    gate.querySelector('#relayAutoLandscapeButton')?.addEventListener('click', async () => {
      ...
      sync();
      if (isPortrait()) {
        fallback.textContent = 'This browser blocked automatic rotation. Install Runner Relay as an app for automatic landscape launch.';
      }
      button.disabled = false;
      button.textContent = isPortrait() ? 'TRY LANDSCAPE AGAIN' : 'LANDSCAPE ACTIVE';
    }, { passive: false });
