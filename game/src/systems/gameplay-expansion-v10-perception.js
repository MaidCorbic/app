...
export function installGameplayExpansionV10(RunnerScene){
 ...
 RunnerScene.prototype.create=function(...args){
  ...
  scene.__v10={state,destroyed:false};
  ...
  const setStatus=t=>status.setText(t),world=[],add=o=>(world.push(o),o);
  ...
  solveSymbol=id=>{if(state.symbols.solved)return;const expected=state.symbols.sequence[state.symbols.progress];if(id===expected){state.symbols.progress++;if(state.symbols.progress===3)state.symbols.solved=true;save(state);setStatus(state.symbols.solved?'SYMBOLS VERIFIED — ROUTE UNLOCKED':`SYMBOL ${state.symbols.progress}/3 VERIFIED`);}else{state.symbols.progress=0;save(state);setStatus('WRONG SYMBOL — SEQUENCE RESET');}};
  ...
  const key=e=>{if(e.repeat)return;switch(e.code){case'KeyM':rotateMirror();break;case'KeyY':solveSymbol(state.symbols.sequence[state.symbols.progress]??0);break;case'KeyU':state.memory.revealed=!state.memory.revealed;landmarks.forEach(c=>c.setAlpha(state.memory.revealed?1:.3));renderMemory();setStatus(state.memory.revealed?'MEMORY LANDMARKS REVEALED':'MEMORY LANDMARKS HIDDEN');break;case'KeyP':capture();break;}};
