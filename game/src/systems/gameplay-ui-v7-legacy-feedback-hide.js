...
(() => {
  ...
  const TEXT_RE =
    /DYNAMIC\s+CROWD|CROWD\s+FEEDBACK|DYNAMIC\s+FEEDBACK|CHECKPOINT\s+(SECURED|RESTORED|SAVED)|RESPAWN\s+LINK\s+ACTIVE|MISSION\s+INTELLIGENCE|LIVE\s+MISSION\s+INTEL|SIGNAL\s+(COLLECTED|SECURED|CAPTURED|RECOVERED)|^V10\b|^ALT\+Q\s*\/\s*W\s*\/\s*E\b/i;
  ...
  const inspect = node => {
    ...
    let matched =
      Boolean(text && TEXT_RE.test(text));
    ...
    if (Array.isArray(node.list)) {
      ...
      if (!matched && childTexts.length) {
        matched = childTexts.some(
          value => TEXT_RE.test(value)
        );
      }
    }
    ...
    if (matched) {
      hideNode(
        node.parentContainer?.setVisible
