...
(() => {
  ...
  const resetTransientState = scene => {
    ...
    // Movement systems use these flags as frame-to-frame state. Clear only
    // transient values; persistent progression/checkpoint state is untouched.
    for (const key of [
  ...
  RunnerScene.prototype.fail = function deathRetryStateResetFail(...args) {
    // Clear held input immediately when death is accepted. The existing death
    // animation/fail flow remains responsible for timing, rewards and respawn.
