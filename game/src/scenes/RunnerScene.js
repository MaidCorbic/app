...
export class RunnerScene extends Phaser.Scene {
  ...
  init({ mission, runId, abilities = [], rain, screenShake = true, reducedMotion = false, firstTimeTutorial = false }) {
    this.mission = mission; this.runId = runId; this.abilities = new Set(abilities); this.rainEnabled = rain; this.screenShake = screenShake; this.motionReduced = reducedMotion; this.firstTimeTutorial = firstTimeTutorial; this.collected = 0; this.secretsCollected = 0; this.elapsedMs = 0; this.timeEmitTimer = 0; this.boostCooldown = 0; this.dashCooldown = 0; this.dashTimer = 0; this.wallJumpCooldown = 0; this.wallJumpTimer = 0; this.lowEnergyCueTimer = 0; this.health = 3; this.healthInvulnerable = 0; this.ammo = 6; this.ammoMax = 6; this.ammoRecharge = 0; this.cometTimer = 3400; this.blasterCooldown = 0; this.swordCooldown = 0; this.buildCooldowns = [0, 0]; this.combatCombo = 0; this.comboTimer = 0; this.jumps = 0; this.collisions = 0; this.falls = 0; this.jumpsUsed = 0; this.finished = false; this.respawning = false; this.respawnGrace = 0; this.cinematicActive = mission.id === 'first-delivery' && firstTimeTutorial; this.coyote = 0; this.jumpBuffer = 0; this.dustTimer = 0; this.speedTimer = 0; this.lastProgress = -1; this.wasGrounded = false; this.fallSpeed = 0; this.cameraOffsetX = -85; this.cameraOffsetY = 65; this.cameraZoom = 1; this.jumpHeld = false; this.sectorTwoAnnounced = false; this.chaseWarnings = new Set(); this.checkpointHints = new Set(); this.routeTutorials = new Set(); this.storyBeatsSeen = new Set(); this.enemyIntelSeen = new Set(); this.goalHintShown = false; this.checkpoint = { x: mission.spawn.x, y: mission.spawn.y, signals: new Set(), secrets: new Set() };
  }
  ...
  showIntelCard(title, lines, color = '#8df4ff') {
    this.infoCard?.destroy(true); const card = this.add.container(32, 382).setScrollFactor(0).setDepth(40); const plate = this.add.rectangle(236, 112, 472, 210, 0x07101f, .94).setStrokeStyle(1, Phaser.Display.Color.HexStringToColor(color).color, .8); const heading = this.add.text(28, 20, title, { fontFamily: 'DM Mono', fontSize: '13px', color }); const divider = this.add.rectangle(28, 47, 74, 2, Phaser.Display.Color.HexStringToColor(color).color); const copy = this.add.text(28, 65, lines.join('\n'), { fontFamily: 'DM Mono', fontSize: '11px', color: '#dffcff', lineSpacing: 9, wordWrap: { width: 410 } }); const dismiss = this.add.text(28, 174, 'RUN TO DISMISS', { fontFamily: 'DM Mono', fontSize: '9px', color: '#8ba0b8' }); card.add([plate, heading, divider, copy, dismiss]); card.setAlpha(0); this.tweens.add({ targets: card, alpha: 1, x: 48, duration: 220 }); this.infoCard = card; this.time.delayedCall(4200, () => { if (this.infoCard === card) { this.tweens.add({ targets: card, alpha: 0, duration: 240, onComplete: () => card.destroy() }); this.infoCard = null; } });
  }
  showEnemyIntel(type) { const intel = enemyIntel[type]; if (!intel || this.enemyIntelSeen.has(type)) return; this.enemyIntelSeen.add(type); this.game.events.emit('enemy-discovered', type); if (!this.firstTimeTutorial) return; this.showIntelCard(`ENEMY INTEL · ${intel.name}`, [`ATTACK · ${intel.attack}`, `DEFENSE · ${intel.defense}`, `TACTIC · ${intel.tactic}`], type === 'dino-boss' ? '#ffcf82' : '#ff826e'); }
  ...
  create() {
    ...
    this.createEnvironment(); this.createPlatforms(); this.createWorldLandmarks(); this.createRouteLighting(); this.createPlayer(); this.createRival(); this.createSignals(); this.createSecrets(); this.createCheckpoints(); this.createHazards(); this.createMovingGates(); this.createEnemies(); this.createSciFiThreats(); this.createBuildSystems(); this.createBoostPads(); this.createChaser(); this.createGoal(); this.createAtmosphere(); this.createGuides();
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => { this.tweens.killAll(); this.time.removeAllEvents(); this.eventState.clear(); });
  ...
  createSciFiThreats() {
    ...
    if (tier >= 3) { this.boss = addThreat('dino-boss', this.mission.goal.x - 250, this.mission.spawn.y - 12); this.boss.setData('health', tier >= 6 ? 10 : 7); this.boss.setData('boss', true); const name = this.add.text(this.boss.x, this.boss.y - 72, 'ALPHA DINO · CLEAR THE ROUTE', { fontFamily: 'DM Mono', fontSize: '10px', color: '#ffcf82', stroke: '#08101c', strokeThickness: 4 }).setOrigin(.5).setDepth(13); this.boss.setData('label', name); }
    if (this.mission.id === 'first-delivery') { const label = (enemy, text) => this.add.text(enemy.x, enemy.y - 62, text, { fontFamily: 'DM Mono', fontSize: '10px', color: '#b9f5ff', stroke: '#08101c', strokeThickness: 4, align: 'center' }).setOrigin(.5).setDepth(13); label(firstRunner, 'SCOUT RUNNER\nE · FIRE OR STOMP'); label(firstChicken, 'EGG HAZARD\nJUMP OR SHOOT'); }
  ...
  takeSciFiHit(message) { if (this.respawning || this.finished || this.healthInvulnerable > 0) return; this.health--; this.healthInvulnerable = 1100; if (this.health <= 0) { this.health = 3; message = 'The courier collapsed. Checkpoint health restored.'; } this.game.events.emit('health', this.health); this.fail(message); }
  useBuild(slot) {
  ...
  defeatEnemy(enemy, method, power = 1) { if (!enemy?.active) return; const health = enemy.getData('health'); if (health) { const remaining = health - power; enemy.setData('health', remaining); enemy.setTint(0xff826e); this.time.delayedCall(100, () => enemy.active && enemy.clearTint()); if (remaining > 0) { enemy.getData('label')?.setText(`ALPHA DINO · ${remaining} HITS LEFT`); this.playerCue(`${method} · BOSS HIT`, '#ffcf82'); return; } enemy.getData('label')?.destroy(); } enemy.getData('indicator')?.destroy(); const burst = this.add.circle(enemy.x, enemy.y, 12, 0x8df4ff, .65).setDepth(13); this.tweens.add({ targets: burst, scale: 3, alpha: 0, duration: 220, onComplete: () => burst.destroy() }); enemy.disableBody(true, true); this.enemyDefeats = (this.enemyDefeats || 0) + 1; this.combatCombo = this.comboTimer > 0 ? this.combatCombo + 1 : 1; this.comboTimer = 3000; this.ammo = Math.min(this.ammoMax, this.ammo + 1); this.game.events.emit('ammo', this.ammo / this.ammoMax * 100); this.player.body.setVelocityY(method === 'STOMP' ? -360 : this.player.body.velocity.y); this.playerCue(`${method} · ${this.combatCombo > 1 ? `COMBO x${this.combatCombo}` : 'THREAT CLEARED'}`, '#8df4ff'); }
  ...
  createOpeningCinematic() {
    const finish = () => { if (!this.cinematicActive) return; this.cinematicActive = false; overlay.destroy(true); this.playerCue('LANDING COMPLETE · E TO FIRE · STOMP FROM ABOVE', '#8df4ff'); };
    ...
    const visuals = [planetGlow, ...stars, atmosphere, planet];
    ...
    this.time.delayedCall(compact ? 3600 : 5600, finish); this.input.keyboard.once('keydown-SPACE', finish);
  }
  ...
  updateEnemies(delta) {
    ...
    this.enemies.getChildren().forEach(enemy => { if (!enemy.active) return; const route = enemy.getData('route'); const type = route.type; let direction = enemy.getData('direction'); const horizontalDistance = Math.abs(enemy.x - this.player.x); const verticalDistance = Math.abs(enemy.y - this.player.y); const speed = type === 'dino' ? 74 : type === 'enemy-runner' && horizontalDistance < 300 ? 118 : type === 'invader' ? 28 : type === 'security' ? 55 : 38; if (type === 'enemy-runner' && horizontalDistance < 300) direction = this.player.x < enemy.x ? -1 : 1; enemy.x += direction * speed * delta / 1000; if (enemy.x >= route.max || enemy.x <= route.min) { direction *= -1; enemy.setData('direction', direction); enemy.setFlipX(direction < 0); } enemy.body.updateFromGameObject();
      const patrol = type === 'security' || type === 'guard'; const detectionRange = this.mission.blackout && this.player.y < 470 ? 105 : type === 'security' ? 180 : 145; const indicator = enemy.getData('indicator'); indicator?.setPosition(enemy.x, enemy.y - 30).setAlpha(horizontalDistance < Math.max(260, detectionRange * 1.6) && verticalDistance < 150 ? .92 : .28).setFillStyle(horizontalDistance < detectionRange && verticalDistance < 100 ? 0xffd06e : 0xff826e); if (patrol && horizontalDistance < detectionRange && verticalDistance < 100) { if (this.alarmTimer <= 0) { this.alarms++; this.game.events.emit('feedback', 'chase'); } this.alarmTimer = this.alarmDuration(3400); }
    });
  ...
  updateSciFiThreats(delta) {
    ...
    this.enemies.getChildren().forEach(enemy => { if (!enemy.active) return; const type = enemy.getData('route')?.type; if (type === 'chicken' || type === 'invader' || type === 'enemy-runner') { if (this.player.x > enemy.x - 430 && this.player.x < enemy.x + 430 && now >= enemy.getData('nextShot')) { const projectile = type === 'chicken' ? 'egg' : 'comet'; const egg = this.eggs.create(enemy.x, enemy.y + 12, projectile).setDepth(11); egg.body.setAllowGravity(false).setVelocity((this.player.x - enemy.x) * .72, type === 'chicken' ? 210 : 90); enemy.setData('nextShot', now + (type === 'enemy-runner' ? 1500 : type === 'invader' ? 1900 : 2400)); this.playerCue(type === 'chicken' ? 'EGG INCOMING' : 'ENEMY FIRE', '#ff826e'); this.game.events.emit('feedback', 'warning'); } } if (type === 'invader') { enemy.y += Math.sin((now + enemy.x) / 260) * .22; enemy.body.updateFromGameObject(); } if (type === 'dino' && Math.abs(enemy.x - this.player.x) < 240 && Math.abs(enemy.y - this.player.y) < 95 && now >= enemy.getData('nextShot')) { enemy.setData('nextShot', now + 1700); enemy.setTint(0xff826e); this.tweens.add({ targets: enemy, x: this.player.x, duration: 340, onComplete: () => enemy.clearTint() }); if (Math.abs(enemy.x - this.player.x) < 66) this.takeSciFiHit('A dinosaur charge knocked the courier down.'); } });
  ...
  complete() {
    if (this.finished) return; if (this.boss?.active) { this.playerCue('ALPHA DINO BLOCKS THE RELAY · DEFEAT IT', '#ffcf82'); this.player.body.setVelocityX(-260); return; } this.finished = true; this.physics.pause(); this.player.play('runner-finish', true); this.player.setTint(0xffefad); if (!this.motionReduced) this.cameras.main.flash(260, 255, 208, 110); this.game.events.emit('feedback', 'complete'); if (this.mission.story?.completion) { const epilogue = this.add.text(this.goal.x + 20, this.goal.y - 88, this.mission.story.completion, { fontFamily: 'DM Mono', fontSize: '11px', color: '#dffcff', stroke: '#08101c', strokeThickness: 4, wordWrap: { width: 380 }, align: 'center' }).setOrigin(.5).setDepth(14); this.tweens.add({ targets: epilogue, alpha: 0, delay: 2800, duration: 500, onComplete: () => epilogue.destroy() }); }
  ...
  respawnCheckpoint() {
    ...
    this.collected = this.checkpoint.signals.size; this.secretsCollected = this.checkpoint.secrets.size; this.player.clearTint().setPosition(this.checkpoint.x, this.checkpoint.y).play('runner-idle', true); this.player.body.reset(this.checkpoint.x, this.checkpoint.y); this.player.body.setVelocity(0, 0); this.respawnGrace = 1100; if (this.loadout.upgrades?.includes('recovery')) { this.energy = Math.min(this.energyMax, this.energy + 20); this.playerCue('RECOVERY +20 ENERGY', '#aee37f'); } const shield = this.add.circle(this.player.x, this.player.y, 22, 0x8df4ff, .22).setDepth(11); this.tweens.add({ targets: shield, scale: 2.5, alpha: 0, duration: 420, onComplete: () => shield.destroy() }); this.player.setAlpha(.45); this.tweens.add({ targets: this.player, alpha: 1, duration: 260 }); this.playerCue('ROUTE RESET · SAFE PLATFORM', '#b9f5ff'); this.physics.resume(); this.respawning = false;
