      ...
      <section id="playerProfile" aria-label="Player profile and progression">
        <div class="pp-shell">
          <header class="pp-head">
            <div class="pp-head-copy"><p class="pp-kicker">COURIER PROFILE // LIVE STATE</p><h2 class="pp-title">PLAYER <em>PROGRESSION.</em></h2><p class="pp-sub">Live profile data from the current game save.</p></div>
          ...
          <section class="pp-level" aria-label="Level progress">
            <div class="pp-level-label"><small>LEVEL</small><b>${level.level}</b></div>
            <div class="pp-xp-wrap"><div class="pp-xp-track" aria-hidden="true"><i class="pp-xp-fill" style="width:${pct(level.progress)}"></i></div><div class="pp-xp-copy"><b>${number(xpIntoLevel)}</b> / ${number(xpNeeded)} XP · ${xpRemaining ? `${number(xpRemaining)} TO NEXT` : 'MAX LEVEL'}</div></div>
          </section>
          ...
          <footer class="pp-footer">PROFILE V1 · READ-ONLY · GAMEPLAY AND SAVE LOGIC UNCHANGED</footer>
        </div>
      </section>`;
  }
